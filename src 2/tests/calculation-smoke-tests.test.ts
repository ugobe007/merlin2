...
// SSOT Services
import { calculateUseCasePower, POWER_DENSITY_STANDARDS, HOTEL_CLASS_PROFILES, HOTEL_AMENITY_SPECS } from '@/services/useCasePowerCalculations';
...
describe('SSOT Constants Consistency', () => {
  ...
  test('POWER_DENSITY_STANDARDS has all required use cases', () => {
    const requiredKeys = [
      'office', 'retail', 'shoppingCenter', 'warehouse', 
      'manufacturing', 'coldStorage', 'datacenter', 'indoorFarm',
      'casino', 'hotelPerRoom', 'hospitalPerBed', 'agriculturePerAcre',
      'airportPerMillion'
    ];
    
    requiredKeys.forEach(key => {
      expect(POWER_DENSITY_STANDARDS).toHaveProperty(key);
      expect(typeof POWER_DENSITY_STANDARDS[key as keyof typeof POWER_DENSITY_STANDARDS]).toBe('number');
      expect(POWER_DENSITY_STANDARDS[key as keyof typeof POWER_DENSITY_STANDARDS]).toBeGreaterThan(0);
    });
  });
  ...
  test('HOTEL_AMENITY_SPECS has required amenities', () => {
    const amenities = ['pool', 'restaurant', 'spa', 'fitnessCenter', 'evCharging', 'laundry', 'conferenceCenter'];
...
describe('Equipment Calculations', () => {
  ...
  test('Larger system = higher cost', async () => {
    const small = await calculateEquipmentBreakdown(
      0.5, 4, 0, 0, 0, undefined, 'on-grid', 'California'
    );
    
    const large = await calculateEquipmentBreakdown(
      2.0, 4, 0, 0, 0, undefined, 'on-grid', 'California'
    );
...
describe('Full Quote Flow (Integration)', () => {
  ...
  test('calculateQuote produces complete quote for hotel', async () => {
    ...
    console.log(`✅ Hotel quote: $${result.costs.netCost?.toLocaleString()} net, ${result.financials.paybackYears?.toFixed(1)} yr payback`);
  });
  ...
  test('calculateQuote produces complete quote for car wash', async () => {
    ...
    console.log(`✅ Car wash quote: $${result.costs.netCost?.toLocaleString()} net, ${result.financials.paybackYears?.toFixed(1)} yr payback`);
  });
  ...
  test('calculateQuote produces complete quote for EV charging', async () => {
    ...
    console.log(`✅ EV charging quote: $${result.costs.netCost?.toLocaleString()} net, ${result.financials.paybackYears?.toFixed(1)} yr payback`);
  });
});
