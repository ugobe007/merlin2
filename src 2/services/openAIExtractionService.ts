...
function localExtraction(documents: ParsedDocument[]): ExtractedSpecsData {
  ...
  const demandMatch = combinedText.match(/(?:peak|demand|max)\s*(?:demand|load|power)?:?\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*kw/i);
  if (demandMatch) {
  ...
  const demandChargeMatch = combinedText.match(/\$?(\d{1,2}(?:\.\d{2})?)\s*(?:per\s+)?(?:\/)?kw(?:\s+demand)?/i);
  ...
  const stateMatch = combinedText.match(/\b(AL|AK|AZ|AR|CA|CO|CT|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY)\b/);
  if (stateMatch) {
    result.location = {
      ...result.location,
      state: stateMatch[1]
  ...
  const solarMatch = combinedText.match(/(?:solar|pv)\s*(?:capacity|system|array)?:?\s*(\d{1,4}(?:\.\d+)?)\s*kw/i);
  if (solarMatch) {
...
export async function extractSpecsFromDocuments(
  ...
  if (apiKey && apiKey !== 'your-key-here') {
    try {
      ...
      if (import.meta.env.DEV) {
        console.log(`🤖 [OpenAIExtraction] AI extraction complete, confidence: ${result.confidence}%`);
      }
...
export function formatExtractedData(data: ExtractedSpecsData): string {
  ...
  if (data.location?.state) {
    lines.push(`Location: ${data.location.city ? data.location.city + ', ' : ''}${data.location.state}`);
  }
