...
const STATE_EXECUTION_SCORES: Record<string, number> = {
  ...
  'TX': 92, 'FL': 88, 'AZ': 90, 'NV': 91, 'GA': 85, 'NC': 84, 'TN': 86,
  'SC': 83, 'AL': 82, 'OK': 87, 'CO': 85, 'UT': 86, 'ID': 84,
  // Moderate states (score 65-79)
  'CA': 72, 'NY': 68, 'NJ': 70, 'MA': 71, 'CT': 69, 'PA': 75, 'OH': 76,
  'IL': 74, 'MI': 73, 'WA': 70, 'OR': 71, 'VA': 77, 'MD': 72, 'MN': 74,
...
const RATE_VOLATILITY: Record<string, 'low' | 'medium' | 'high'> = {
  'TX': 'high', 'CA': 'high', 'NY': 'medium', 'FL': 'medium',
  'AZ': 'medium', 'NV': 'medium', 'HI': 'high', 'AK': 'high',
};
...
function calculateFinancialScore(input: RAVSInput): ComponentResult {
  ...
  const npvScore = Math.min(100, Math.max(0, 
    npvRatio > 1 ? 95 :
    npvRatio > 0.5 ? 85 :
    npvRatio > 0.2 ? 75 :
    npvRatio > 0 ? 65 :
  ...
  const irrScore = Math.min(100, Math.max(0,
    irr >= 20 ? 98 :
    irr >= 15 ? 90 :
    irr >= 12 ? 82 :
    irr >= 10 ? 75 :
  ...
  const paybackScore = Math.min(100, Math.max(0,
    paybackYears <= 3 ? 98 :
    paybackYears <= 4 ? 92 :
    paybackYears <= 5 ? 85 :
    paybackYears <= 6 ? 78 :
...
function calculateMarketScore(input: RAVSInput): ComponentResult {
  ...
  const { electricityRate, incentivesAvailable, itcPercentage, stateIncentives, netMeteringAvailable } = input.market;
  
  // Electricity rate favorability
  // Higher rates = better economics for solar/storage
  const rateScore = Math.min(100, Math.max(40,
    electricityRate >= 0.25 ? 98 :
    electricityRate >= 0.20 ? 92 :
    electricityRate >= 0.15 ? 85 :
    electricityRate >= 0.12 ? 75 :
    electricityRate >= 0.10 ? 65 :
    electricityRate >= 0.08 ? 55 :
    45
  ));
  factors.push({
    ...
    score: rateScore,
    weight: 0.30,
    impact: electricityRate >= 0.15 ? 'positive' : electricityRate >= 0.10 ? 'neutral' : 'negative',
...
function calculateOperationalScore(input: RAVSInput): ComponentResult {
  ...
  const { warrantyYears, expectedLifeYears, maintenanceIncluded, equipmentTier } = input.operational;
  
  // Warranty coverage score
  const warrantyScore = Math.min(100, Math.max(40,
    warrantyYears >= 25 ? 95 :
    warrantyYears >= 20 ? 90 :
    warrantyYears >= 15 ? 82 :
    warrantyYears >= 10 ? 72 :
...
function getConfidenceLevel(interval: { low: number; mid: number; high: number }): 'high' | 'medium' | 'low' {
...
function generateInsights(
  ...
  if (input.operational.installerTier === 'premium' && input.operational.equipmentTier === 'tier1') {
