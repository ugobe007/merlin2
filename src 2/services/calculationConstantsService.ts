...
const FALLBACK_CONSTANTS: Record<string, { value: number; source: string }> = {
  ...
  // Sizing
  peak_shaving_target_percent: { value: 0.30, source: 'Best practice' },
 ...
 */
export async function getConstantsByCategory(
  category: string
  ...
  Object.entries(FALLBACK_CONSTANTS).forEach(([key, data]) => {
    if (!result.has(key)) {
      ...
      const inferredCategory = key.includes('battery') || key.includes('solar') || key.includes('inverter') || key.includes('installation')
        ? 'pricing'
        : key.includes('itc') || key.includes('discount') || key.includes('lifetime') || key.includes('degradation') || key.includes('escalation')
        ? 'financial'
        : 'sizing';
...
export async function getITCRate(): Promise<number> {
  return await getConstant('federal_itc_rate') || 0.30;
}
...
export async function getPricingConstants(): Promise<{
  ...
  return {
    ...
    solarPerWatt: solarPerWatt || 2.50,
