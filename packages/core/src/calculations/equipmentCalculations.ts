...
// ✅ Using database pricing tiers with market intelligence fallback

import { calculateMarketAlignedBESSPricing, getMarketIntelligenceRecommendations } from '../pricing/marketIntelligence';
...
export const calculateEquipmentBreakdown = async (
  ...
  // ✅ INTEGRATED: Use database pricing tiers with market intelligence fallback
  ...
  try {
    ...
    if (pricingTier && pricingTier.price > 0) {
      ...
      pricingTierSource = pricingTier.tier.data_source || pricingTier.tier.config_key || 'database_pricing_tier';
      
      if (process.env.NODE_ENV === "development") {
        console.log(`✅ [Pricing Tier] Using database tier: $${pricingTier.price}${pricingTier.unit} (${pricingTier.tier.config_key})`);
      }
    } else {
      throw new Error('No pricing tier found in database');
    }
  } catch (error) {
    // Fallback to market intelligence pricing
  ...
  const batteries = {
    ...
    marketIntelligence: {
      ...
      dataSource: pricingTierSource || (isSmallSystem ? 'Q4 2024 Market Reality (C&I)' : 'Q4 2024 Market Reality (Utility)')
    }
  };
  ...
  try {
    ...
    // powerElectronicsConfig = await useCaseService.getPricingConfig('power_electronics_2025');
      powerElectronicsConfig = null; // Using industry-validated fallback values
  } catch (error) {
  ...
  if (effectiveGeneratorMW > 0) {
    ...
    try {
      ...
      const generatorConfig: any = null; // Using industry-validated fallback values
      if (generatorConfig) {
  ...
  let solar = undefined;
  if (solarMW > 0) {
    // ✅ INTEGRATED: Use database-driven size-based pricing tiers with fallback
    let costPerWatt: number;
    let priceSource = 'database_pricing_tier';
    
    try {
      const sizeKW = solarMW * 1000;
      const pricingTier = await getPricingTier('solar_pv', sizeKW, null, 'mid');
      
      if (pricingTier && pricingTier.price > 0) {
        // Convert price from $/W to cost per watt
        costPerWatt = pricingTier.unit === '$/W' ? pricingTier.price : pricingTier.price;
        priceSource = pricingTier.tier.data_source || pricingTier.tier.config_key || 'database_pricing_tier';
        
        if (process.env.NODE_ENV === "development") {
          console.log(`✅ [Solar Pricing Tier] Using database tier: $${costPerWatt}${pricingTier.unit} (${pricingTier.tier.config_key})`);
        }
      } else {
        throw new Error('No solar pricing tier found in database');
      }
    } catch (error) {
      // Fallback to validated quote pricing
      console.warn('⚠️ [Solar Pricing Tier] Database query failed, using validated quote fallback:', error);
      const isUtilityScale = solarMW >= 5; // 5MW+ is utility scale
      costPerWatt = isUtilityScale ? 0.65 : 1.05; // Validated: Hampton Heights $0.60/W, Tribal Microgrid $1.05/W
      priceSource = isUtilityScale ? 'Utility Solar (≥5 MW) - Validated Quote Fallback' : 'Commercial Solar (<5 MW) - Validated Quote Fallback';
    }
  ...
  if (windMW > 0) {
    ...
    try {
      ...
      const windConfig: any = null; // Using industry-validated fallback values
      if (windConfig) {
  ...
  if (fuelCellMW > 0) {
    ...
    try {
      ...
      const fuelCellConfig: any = null; // Using industry-validated fallback values
      if (fuelCellConfig) {
  ...
  if (industryData?.selectedIndustry === 'ev-charging' && industryData?.useCaseData) {
    ...
    try {
      ...
      const evConfig: any = null; // Using industry-validated fallback values
      if (evConfig) {
  ...
  try {
    // const bopConfig = await import('../services/useCaseService').then(m => m.useCaseService.getPricingConfig('balance_of_plant_2025'));
      const bopConfig: any = null; // Using industry-validated fallback values
    if (bopConfig) {
  ...
  try {
    ...
    // commissioningConfig = await useCaseService.getPricingConfig('commissioning_costs_2025');
      commissioningConfig = null; // Using industry-validated fallback values
  } catch (error) {
  ...
  try {
    ...
    // certificationConfig = await useCaseService.getPricingConfig('certification_costs_2025');
      certificationConfig = null; // Using industry-validated fallback values
  } catch (error) {
