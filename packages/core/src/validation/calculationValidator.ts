...
// Alert notification service - optional dependency, can be added later
...
export const BENCHMARK_BOUNDS = {
  ...
  solar: {
    ...
    max: 2.50,     // Small commercial with soft costs
  ...
  // ITC Rate
  itcRate: {
    min: 0.30,     // Standard ITC
    max: 0.50,     // With adders (domestic content, energy community)
    typical: 0.30, // Standard rate
    source: 'IRA 2022',
  },
};
...
export const USE_CASE_BOUNDS: Record<string, {
  minKW: number;
  maxKW: number;
  typicalKW: number;
  description: string;
...
export class CalculationValidator {
  ...
  static async validateQuote(
    ...
    if (equipmentCost > 0 && expectedEquipmentCost > 0) {
      const costDeviation = Math.abs((equipmentCost - expectedEquipmentCost) / expectedEquipmentCost) * 100;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // TODO: Add alert notification service integration
