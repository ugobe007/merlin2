  ...
  Object.entries(patterns).forEach(([key, pattern]) => {
    ...
    if (matches.length > 0) {
      ...
      if (key === "warranty" || key === "cycles") {
        specs[key === "warranty" ? "warrantyYears" : "cycleLife"] = parseInt(
          matches[0][1].replace(",", "")
        );
  ...
  try {
    for (const trend of trends) {
      const { error } = await supabase.from("ai_training_data").insert({
        ...
        source: "rss_feed",
        confidence_score:
          trend.impactLevel === "high" ? 0.9 : trend.impactLevel === "medium" ? 0.7 : 0.5,
        created_at: new Date().toISOString(),
      });
...
export async function processBatchForAI(articles: RSSArticle[]): Promise<{
  ...
  if (import.meta.env.DEV) {
    console.log(
      `   📊 Total: ${totalPricing} pricing, ${totalConfig} configs, ${totalTrends} trends`
    );
  }
...
export async function getAIDataStats(): Promise<{
  ...
  if (!supabase) {
    ...
    const keys = Object.keys(localStorage).filter(
      (k) => k.startsWith("pricing_") || k.startsWith("config_") || k.startsWith("trends_")
    );
    return {
