 ...
 */
export function generateRecommendations(input: RecommendationEngineInput): OpportunityRecommendation {
  const {
    state,
    gridConnection,
    electricityRate,
  ...
  const rooftopSqFt = parseFloat(useCaseData.rooftopSquareFootage || useCaseData.rooftopSqFt || '0');
  const totalSqFt = parseFloat(useCaseData.totalFacilitySquareFootage || useCaseData.totalFacilitySqFt || useCaseData.totalSqFt || '0');
  const isDowntown = useCaseData.locationType === 'downtown' || useCaseData.urbanSetting === 'downtown';
  const isEVChargingHub = industry.toLowerCase().includes('ev-charging') || industry.toLowerCase().includes('ev-charging-hub');
...
function recommendSolar(params: {
  ...
  // Space analysis
  const spaceAnalysis = rooftopSqFt > 0 ? {
  ...
  if (rooftopSqFt < 500) {
    constraints.push('Insufficient rooftop space (<500 sq ft) - minimum needed for viable solar installation');
    if (isDowntown || isEVChargingHub) {
      constraints.push(`${isEVChargingHub ? 'EV charging hub' : 'Downtown location'} typically lacks adequate rooftop space for solar`);
    }
  } else if (!feasible) {
    constraints.push(`Required space (${Math.round(requiredSqFt).toLocaleString()} sq ft) exceeds available rooftop (${Math.round(rooftopSqFt).toLocaleString()} sq ft)`);
  }

  // Constraint 2: Poor solar rating
  if (peakSunHours < 3.5) {
    constraints.push(`Poor solar rating (${peakSunHours.toFixed(1)} peak sun hours/day) - below recommended threshold of 3.5 hours`);
  }

  // Constraint 3: Off-grid or unreliable grid (generators better)
  if (gridConnection === 'off-grid' || gridConnection === 'unreliable') {
    constraints.push(`${gridConnection === 'off-grid' ? 'Off-grid' : 'Unreliable grid'} - backup generators provide more reliable power than solar`);
    alternativeSuggestion = 'Backup Generator';
  }

  // Constraint 4: Low electricity rates (poor ROI)
  if (electricityRate < 0.08) {
    constraints.push(`Low electricity rates ($${electricityRate.toFixed(3)}/kWh) - ROI period would exceed 10+ years`);
  }

  // Advantage 1: Good solar rating
  if (peakSunHours >= 4.0) {
    advantages.push(`Excellent solar potential (${peakSunHours.toFixed(1)} peak sun hours/day, ${solarRating} rating)`);
  }

  // Advantage 2: Adequate space
  if (rooftopSqFt >= 2000 && feasible) {
    advantages.push(`Adequate rooftop space (${Math.round(rooftopSqFt).toLocaleString()} sq ft) - can support ~${estimatedCapacityKW} kW solar array`);
  }

  // Advantage 3: High electricity rates (good ROI)
  if (electricityRate >= 0.12) {
    advantages.push(`High electricity rates ($${electricityRate.toFixed(3)}/kWh) - excellent ROI potential`);
  }
...
function recommendGenerator(params: {
  ...
  if (gridConnection === 'off-grid' || gridConnection === 'unreliable') {
    advantages.push(`${gridConnection === 'off-grid' ? 'Off-grid' : 'Unreliable grid'} - generators provide essential backup power`);
    recommended = true;
  }
  ...
  if (gridConnection === 'on-grid' && !goals.includes('backup-power') && !goals.includes('grid-independence')) {
...
function recommendEVCharging(params: {
  ...
  if (isDowntown || industry.toLowerCase().includes('retail') || industry.toLowerCase().includes('hotel')) {
  ...
  if (electricityRate >= 0.12) {
    advantages.push(`High electricity rates ($${electricityRate.toFixed(3)}/kWh) - good arbitrage opportunity with BESS`);
    recommended = recommended || true;
  }
  ...
  if (electricityRate < 0.08) {
    constraints.push(`Low electricity rates ($${electricityRate.toFixed(3)}/kWh) - limited arbitrage opportunity`);
    recommended = false;
  }
