...
export function calculateCO2Avoided(input: CO2CalculationInput): EnvironmentalMetrics {
  ...
  switch (systemType) {
    case 'bess':
      ...
      co2AvoidedKgPerYear = annualKWhDisplaced * gridEmissionFactor * 0.20;
      break;
  ...
  const equivalencies = {
    ...
    carsOffRoad: Math.round(co2AvoidedTonsPerYear * EMISSION_EQUIVALENCIES.carsOffRoadPerTonCO2 * 10) / 10,
    homesElectricity: Math.round(co2AvoidedTonsPerYear * EMISSION_EQUIVALENCIES.homeElectricityPerTonCO2 * 10) / 10,
    gasolineGallons: Math.round(co2AvoidedTonsPerYear * EMISSION_EQUIVALENCIES.gasolineGallonsPerTonCO2),
    milesNotDriven: Math.round(co2AvoidedTonsPerYear * EMISSION_EQUIVALENCIES.milesNotDrivenPerTonCO2),
    smartphonesCharged: Math.round(co2AvoidedTonsPerYear * EMISSION_EQUIVALENCIES.smartphonesChargedPerTonCO2),
  };
  ...
  const renewablePercentage = totalEnergy > 0 
    ? Math.round(((annualKWhGenerated || 0) / totalEnergy) * 100)
    : 0;
...
export function quickCO2Estimate(
  ...
  if (systemType === 'ev-charging') {
    ...
    co2TonsPerYear = (annualSavingsKWh * factor * 0.20) / 1000;
  }
