...
async function checkParsingLogic(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const articlesWithPrices = articles.filter(
      (a) =>
        a.prices_extracted && Array.isArray(a.prices_extracted) && a.prices_extracted.length > 0
    );
    const articlesWithTopics = articles.filter(
      (a) => a.topics && Array.isArray(a.topics) && a.topics.length > 0
    );
    const articlesWithEquipment = articles.filter(
      (a) =>
        a.equipment_mentioned &&
        Array.isArray(a.equipment_mentioned) &&
        a.equipment_mentioned.length > 0
    );
...
async function checkTrueQuoteCompliance(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const hasSources =
      testQuote.benchmarkAudit?.sources && testQuote.benchmarkAudit.sources.length > 0;
...
async function checkNestedCalculations(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const status: "pass" | "warning" | "fail" =
      allIssues.length === 0 ? "pass" : allIssues.length <= 2 ? "warning" : "fail";
...
async function checkWizardFunctionality(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const status: "pass" | "warning" | "fail" =
      issues.length === 0 ? "pass" : issues.length <= 1 ? "warning" : "fail";
  ...
  try {
    ...
    const results = await Promise.allSettled(
      testCases.map(async (testCase) => {
        ...
        if (
          quote.financials.npv &&
          (quote.financials.npv < -1000000 || quote.financials.npv > 100000000)
        ) {
    ...
    const status: "pass" | "warning" | "fail" =
      allIssues.length === 0 ? "pass" : allIssues.length <= 1 ? "warning" : "fail";
...
async function checkMerlinMetrics(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    } else if (hoursSinceValidation > 168) {
      // 7 days
      status = "warning";
      score = 70;
