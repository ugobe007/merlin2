 ...
 */
export function calculateSolarBESSSystem(
  inputs: SolarBESSSystemInputs
): SolarBESSSystemResults {
  const {
    dailyLoadkWh,
    peakLoadkW,
    location,
  ...
  const batteryCapacityAh = 
    (dailyWh * autonomyDays * 2 * 1.1) / 
    (systemVoltage * temperatureDerating);
  ...
  const solarPanelWattage = 
    (batteryCapacityAh * systemVoltage) / 
    (peakSunHours * chargeEfficiency);
  ...
  const costs = {
    ...
    solar: actualSolarWattage * 0.50, // $0.50/W for commercial solar
    chargeController: chargeControllerType === 'MPPT' ? 2000 : 500, // MPPT more expensive
    inverter: inverterRating * 0.30, // $0.30/W for inverter
    installation: (actualSolarWattage * 0.50 + batteryCapacitykWh * 250) * 0.15, // 15% labor
    total: 0
  };
...
export function getTemperatureDerating(temperatureC: number): number {
  ...
  if (temperatureC >= 0) return 0.90;   // 90% at 0°C
  if (temperatureC >= -10) return 0.80; // 80% at -10°C
  if (temperatureC >= -20) return 0.65; // 65% at -20°C
  return 0.50; // 50% below -20°C
}
 ...
 */
export function getOptimalTiltAngle(latitude: number, optimize: 'year-round' | 'winter' = 'year-round'): number {
  if (optimize === 'winter') {
 ...
 */
export function calculateWireSize(current: number, distanceFeet: number, voltage: number): {
  awg: number;
  description: string;
} {
