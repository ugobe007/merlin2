...
export async function calculatePowerGap(
  ...
  notes.push(
    `Baseline sizing: ${selectedPowerKW}kW / ${selectedEnergyKWh}kWh / ${selectedDurationHours}hrs`
  );
  ...
  notes.push(
    `Required capacity: ${needed.powerKW}kW / ${needed.energyKWh}kWh / ${needed.durationHours}hrs`
  );
...
function calculateEVChargingNeeds(
  ...
  const neededPowerKW = userSpecifiedKW || baseline.powerMW * 1000;
...
function calculateRetailNeeds(
  ...
  const neededPowerKW = baseline.powerMW * 1000;
  let neededDurationHours = baseline.durationHrs;
