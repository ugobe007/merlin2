...
export interface SolarCalculationInput {
  ...
  useBifacial?: boolean;
  /**
   * Live vendor $/W from approved vendor_products (via vendorProductPricingBridge).
   * When provided, overrides the SSOT SOLAR_COST_PER_WATT constant.
...
const SOLAR_CONSTANTS = {
  ...
  USABLE_ROOF_PERCENT: 0.6, // 60% of roof is typically usable
...
// Fallback state data
const FALLBACK_STATE_DATA: Record<
  string,
  { sunHours: number; capacityFactor: number; rating: string }
> = {
...
const INDUSTRY_SOLAR_CONFIG: Record<
  string,
  {
    recommended: boolean;
...
> = {
  ...
  car_wash: {
    ...
    defaultRoofUtilization: 0.45,
    typicalFootprintSqFt: 5500,
    canopyPotentialKW: 50,
    rationale:
      "Peak wash hours align with solar production. ⚠️ Roof limited to ~37 kW — canopy solar recommended for additional capacity",
  },
  // Hotel: 20,000 sqft footprint × 35% usable (HVAC, pools, penthouses) = 7,000 sqft → ~105 kW
  hotel: {
    recommended: true,
    targetCoveragePercent: 40,
    defaultRoofUtilization: 0.35,
    typicalFootprintSqFt: 20000,
    canopyPotentialKW: 120,
    rationale: "Excellent daytime load match with guest services. Parking canopy adds capacity",
  },
  // Office: 15,000 sqft footprint × 40% (multi-story, HVAC) = 6,000 sqft → ~90 kW
  office: {
    recommended: true,
    targetCoveragePercent: 45,
    defaultRoofUtilization: 0.4,
    typicalFootprintSqFt: 15000,
    canopyPotentialKW: 100,
    rationale: "Daytime occupancy aligns with solar. Multi-story limits roof area",
  },
  // Data Center: 40,000 sqft × 30% (heavy HVAC, generators, switchgear) = 12,000 → ~180 kW
  data_center: {
    recommended: true,
    targetCoveragePercent: 30,
    defaultRoofUtilization: 0.3,
    typicalFootprintSqFt: 40000,
    canopyPotentialKW: 120,
    rationale: "Consistent 24/7 load benefits from solar + BESS. Roof equipment limits capacity",
  },
  // Hospital: 80,000 sqft × 25% (helipad, cooling towers, exhaust stacks) = 20,000 → ~300 kW
  hospital: {
    recommended: true,
    targetCoveragePercent: 25,
    defaultRoofUtilization: 0.25,
    typicalFootprintSqFt: 80000,
    canopyPotentialKW: 200,
    rationale: "Reduces grid dependence for critical facility. Roof limited by medical equipment",
  },
  // Manufacturing: 75,000 sqft × 60% (large flat, some exhaust/cranes) = 45,000 → ~675 kW
  manufacturing: {
    recommended: true,
    targetCoveragePercent: 35,
    defaultRoofUtilization: 0.6,
    typicalFootprintSqFt: 75000,
    canopyPotentialKW: 150,
    rationale: "Large roof area, daytime production shift. Excellent solar candidate",
  },
  // Retail: 50,000 sqft × 70% (big box, clean roofs) = 35,000 → ~525 kW
  retail: {
    recommended: true,
    targetCoveragePercent: 50,
    defaultRoofUtilization: 0.7,
    typicalFootprintSqFt: 50000,
    canopyPotentialKW: 250,
    rationale: "Business hours match solar production. Large clean roofs + parking canopy",
  },
  // Warehouse: 100,000 sqft × 80% (cleanest roofs in commercial) = 80,000 → ~1,200 kW
  warehouse: {
    recommended: true,
    targetCoveragePercent: 60,
    defaultRoofUtilization: 0.8,
    typicalFootprintSqFt: 100000,
    canopyPotentialKW: 50,
    rationale: "Excellent roof area for maximum solar. Best roof-to-load ratio",
  },
  ...
  restaurant: {
    ...
    canopyPotentialKW: 15,
    rationale: "Lunch peak matches solar production. Small roof limits capacity",
  },
  ...
  college: {
    ...
    rationale: "Multiple buildings, educational showcase. Large parking canopy potential",
  },
  university: {
    recommended: true,
    targetCoveragePercent: 35,
  ...
  ev_charging: {
    ...
    canopyPotentialKW: 150,
    rationale: "Solar canopy over charging stalls is primary. Roof is secondary",
  },
  // Cold Storage: 60,000 sqft × 75% (large flat roofs) = 45,000 → ~675 kW
  cold_storage: {
    recommended: true,
    targetCoveragePercent: 40,
    defaultRoofUtilization: 0.75,
    typicalFootprintSqFt: 60000,
    canopyPotentialKW: 50,
    rationale: "Large flat roofs, consistent refrigeration load",
  },
  // Casino: 120,000 sqft × 50% (signage, HVAC, decorative) = 60,000 → ~900 kW
  casino: {
    recommended: true,
    targetCoveragePercent: 35,
    defaultRoofUtilization: 0.5,
    typicalFootprintSqFt: 120000,
    canopyPotentialKW: 400,
    rationale: "24/7 load pairs well with BESS. Large parking canopy potential",
  },
  ...
  apartment: {
    ...
    canopyPotentialKW: 80,
    rationale: "Common area and EV charging potential. Multi-story limits roof",
  },
  // Gas Station: 3,000 sqft × 55% = 1,650 → ~25 kW. Pump canopy is key!
  gas_station: {
    recommended: true,
    targetCoveragePercent: 40,
    defaultRoofUtilization: 0.55,
    typicalFootprintSqFt: 3000,
    canopyPotentialKW: 40,
    rationale: "Pump canopy solar provides shade + power. 24/7 lighting load",
  },
  // Shopping Center: 200,000 sqft × 65% = 130,000 → ~1,950 kW
  shopping_center: {
    recommended: true,
    targetCoveragePercent: 45,
    defaultRoofUtilization: 0.65,
    typicalFootprintSqFt: 200000,
    canopyPotentialKW: 500,
    rationale: "Massive roof + parking. Among best commercial solar candidates",
  },
  // Airport: 200,000 sqft × 20% (complex roofs, security) = 40,000 → ~600 kW
  airport: {
    recommended: true,
    targetCoveragePercent: 20,
    defaultRoofUtilization: 0.2,
    typicalFootprintSqFt: 200000,
    canopyPotentialKW: 500,
    rationale: "Huge parking areas for canopy solar. Complex roof limits rooftop",
  },
  ...
  indoor_farm: {
    ...
    defaultRoofUtilization: 0.25,
    typicalFootprintSqFt: 40000,
    canopyPotentialKW: 30,
    rationale: "May need translucent roof sections for crops. BESS helps with grow lights",
  },
  ...
  government: {
    ...
    canopyPotentialKW: 120,
    rationale: "Federal sustainability mandates. Executive Order 14057 net-zero by 2050",
  },
  ...
  residential: {
    ...
    typicalFootprintSqFt: 2000,
    canopyPotentialKW: 5,
    rationale: "Rooftop solar offsets most residential consumption. Carport adds capacity",
  },
  ...
  agricultural: {
    ...
    typicalFootprintSqFt: 20000,
    canopyPotentialKW: 50,
    rationale: "Barn/shed roofs ideal for solar. Irrigation load alignment",
  },
  ...
  microgrid: {
    ...
    canopyPotentialKW: 50,
    rationale: "Solar essential for islanded operation. Ground mount also common",
  },
};
...
function extractFacilitySize(useCaseData: Record<string, any>, industry?: string): number {
  ...
  if (industry) {
    ...
    if (config?.typicalFootprintSqFt) {
      if (import.meta.env.DEV)
        console.log(
          `🏠 [solarCalculator] Using industry footprint fallback for ${industry}: ${config.typicalFootprintSqFt} sq ft`
        );
 ...
 */
export async function calculateSolarAsync(
...
export function calculateSolar(input: SolarCalculationInput): SolarCalculationResult {
  const fallback = FALLBACK_STATE_DATA[input.state] || {
    sunHours: 5.0,
    capacityFactor: 1500,
    rating: "C",
  };
  ...
  // Override SOLAR_COST_PER_WATT with live vendor pricing when provided
  const constants = input.vendorPricePerWatt
    ? { ...SOLAR_CONSTANTS, COST_PER_WATT: input.vendorPricePerWatt }
    : SOLAR_CONSTANTS;
  return calculateSolarInternal(input, stateData, constants);
}
...
function calculateSolarInternal(
  ...
  idealCapacityKW = Math.max(
    constants.MIN_SIZE_KW,
    Math.min(constants.MAX_SIZE_KW, idealCapacityKW)
  );
  ...
  if (finalCapacityKW < constants.MIN_SIZE_KW) {
    if (finalCapacityKW + canopyRecommendedKW >= constants.MIN_SIZE_KW) {
      ...
      return buildResult(
        ...
        constants,
        stateData,
        `Roof area alone insufficient (${maxRoofCapacityKW} kW). Adding ${canopyRecommendedKW} kW solar canopy to reach viable system size.`
      );
    }
    return buildResult(
      0,
      ...
      constants,
      stateData,
      `Roof area insufficient for commercial solar (${maxRoofCapacityKW} kW max). Consider ground-mount or canopy solar.`
    );
  }
    ...
    breakdown:
      capacityKW > 0
        ? [
            ...
            { component: "Electrical & Wiring", cost: Math.round(totalCost * 0.1) },
