...
export interface GeneratorCalculationInput {
  ...
  /** Vendor-sourced $/kW for diesel generators (overrides COST_PER_KW_DIESEL) */
  vendorPricePerKwDiesel?: number;
  /** Vendor-sourced $/kW for natural gas generators (overrides COST_PER_KW_NATGAS) */
  vendorPricePerKwNatGas?: number;
}
...
const INDUSTRY_GENERATOR_CONFIG: Record<
  string,
  {
    required: boolean;
...
export function calculateGenerator(input: GeneratorCalculationInput): GeneratorCalculationResult {
  ...
  const costPerKw =
    fuelType === "diesel"
      ? (input.vendorPricePerKwDiesel ?? GENERATOR_CONSTANTS.COST_PER_KW_DIESEL)
      : (input.vendorPricePerKwNatGas ?? GENERATOR_CONSTANTS.COST_PER_KW_NATGAS);
  ...
  if (config.required) {
    ...
    capacityKW = Math.max(
      capacityKW,
      input.peakDemandKW * config.criticalLoadPercent + bessChargingKW
    );
  }

  // Apply min/max limits
  capacityKW = Math.max(
    GENERATOR_CONSTANTS.MIN_SIZE_KW,
    Math.min(GENERATOR_CONSTANTS.MAX_SIZE_KW, capacityKW)
  );
...
function roundToStandardSize(kw: number): number {
  const standardSizes = [
    50, 75, 100, 125, 150, 175, 200, 250, 300, 350, 400, 500, 600, 750, 1000, 1250, 1500, 1750,
...
function buildResult(
  ...
  return {
    ...
    breakdown:
      capacityKW > 0
        ? [
            { component: "Generator Unit", cost: Math.round(totalCost * 0.6) },
            { component: "Transfer Switch", cost: Math.round(totalCost * 0.15) },
            { component: "Fuel System", cost: Math.round(totalCost * 0.1) },
...
export function getGeneratorPresetTiers(
  ...
  const enhancedKW = roundToStandardSize(
    Math.round(peakDemandKW * Math.min(config.criticalLoadPercent * 1.5, 0.8))
  );
  ...
  const tiers: GeneratorPresetTier[] = [
    {
      ...
      annualMaintenance: Math.round(
        standardKW * costPerKw * GENERATOR_CONSTANTS.MAINTENANCE_PERCENT
      ),
    ...
    },
    {
      ...
      coveragePercent: Math.min(config.criticalLoadPercent * 1.5, 0.8),
      estimatedCost: Math.round(enhancedKW * costPerKw),
      annualMaintenance: Math.round(
        enhancedKW * costPerKw * GENERATOR_CONSTANTS.MAINTENANCE_PERCENT
      ),
