 ...
 */
export async function getBESSFinancialInputs(
  systemSizeMWh: number = 4
): Promise<BESSFinancialInputs> {
  try {
...
export const USE_CASE_ENERGY_PROFILES: { [key: string]: UseCaseEnergyProfile } = {
  ...
  "vertical-farm": {
    ...
    // Data Sources
    dataSources: ["iFarm Vertical Farm Energy Consumption Research", "Indoor Ag-Con Industry Data"],
  },
};
...
export function calculateBESSSize(params: {
  ...
  const { peakDemandkW, averageDemandkW, dailyEnergyConsumptionkWh, useCase, primaryObjective } =
    params;
...
export function calculateBESSFinancials(inputs: BESSFinancialInputs): {
  ...
  const annualEnergyMWh =
    powerRatingMW * inputs.dailyCycles * 365 * (inputs.systemAvailability / 100);
  const energyRevenue =
    annualEnergyMWh * inputs.electricityRatePerMWh * (inputs.roundTripEfficiency / 100);
  ...
  for (let year = 1; year <= inputs.projectLifetimeYears; year++) {
    ...
    const yearRevenue =
      annualRevenue * degradationFactor * Math.pow(1 + inputs.priceEscalationRate / 100, year - 1);
