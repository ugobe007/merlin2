...
function calculateRelevanceScore(article: any): number {
  ...
  const hoursSincePublished = (Date.now() - new Date(article.published_at || article.scraped_at).getTime()) / (1000 * 60 * 60);
...
export async function getTopEnergyNews(filters: NewsFilters = {}): Promise<EnergyNewsStory[]> {
  ...
  let query = supabase
    ...
    .select('id, title, url, excerpt, content, published_at, scraped_at, equipment_mentioned, topics, relevance_score, source_id')
...
export async function getNewsStats(): Promise<{
  ...
  const [todayResult, weekResult, monthResult, lastScrapedResult] = await Promise.all([
    supabase.from('scraped_articles').select('id', { count: 'exact' }).gte('scraped_at', today.toISOString()),
    supabase.from('scraped_articles').select('id', { count: 'exact' }).gte('scraped_at', week.toISOString()),
    supabase.from('scraped_articles').select('id', { count: 'exact' }).gte('scraped_at', month.toISOString()),
    supabase.from('scraped_articles').select('scraped_at').order('scraped_at', { ascending: false }).limit(1),
  ]);
