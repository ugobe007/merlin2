...
export function generateMagicFitScenarios(inputs: ScenarioInputs): ScenarioConfig[] {
  ...
  scenarios.push({
    ...
    highlights: ["Fastest payback period", "Lowest upfront investment", "Peak shaving savings"],
  ...
  scenarios.push({
    ...
    highlights: ["Best overall ROI", "Solar + backup power", "Merlin's recommendation"],
    reasoning:
      "Balanced approach maximizing ROI while providing backup power and solar generation.",
  });
  ...
  scenarios.push({
    ...
    highlights: ["Maximum long-term savings", "Near grid independence", "Full renewable stack"],
...
export function getScenarioComparison(scenarios: ScenarioConfig[]): ScenarioComparison {
  const sorted = {
    byCost: [...scenarios].sort((a, b) => a.financials.netInvestment - b.financials.netInvestment),
    byPayback: [...scenarios].sort((a, b) => a.financials.paybackYears - b.financials.paybackYears),
    bySavings: [...scenarios].sort(
      (a, b) => b.financials.annualSavings - a.financials.annualSavings
    ),
    byROI: [...scenarios].sort((a, b) => b.financials.roi25Year - a.financials.roi25Year),
  };
...
export const SCENARIO_SIZING_RATIOS = {
  essentials: {
    bessRatio: 0.4, // 40% of peak demand
  ...
  balanced: {
    bessRatio: 0.7, // 70% of peak demand
    duration: 4, // 4-hour system
    solarRatio: 0.5, // 50% of peak
    windRatio: 0, // No wind
    generatorRatio: 0.27, // 27% for backup
    target: "roi",
  },
  "max-savings": {
    bessRatio: 1.0, // 100% of peak demand
    duration: 6, // 6-hour system
    solarRatio: 0.8, // 80% of peak
    windRatio: 0.1, // 10% wind
...
export function getScenarioDescription(type: ScenarioType): string {
  const descriptions: Record<ScenarioType, string> = {
    essentials:
      "Battery storage only, optimized for fastest return on investment through peak shaving.",
    balanced: "Comprehensive solution with battery, solar, and backup generator for optimal ROI.",
    "max-savings":
      "Full renewable energy stack designed for maximum long-term savings and grid independence.",
  };
