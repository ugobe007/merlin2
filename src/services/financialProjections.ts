...
export const FINANCIAL_DEFAULTS = {
  ...
  discountRate: 0.06, // 6% discount rate for NPV (NREL commercial default)
  ...
  batteryReplacementCostPercent: 0.4, // 40% of original battery cost
};
...
export function calculateFinancialProjection(
  ...
  // Apply defaults
  const electricityEscalation =
    inputs.electricityEscalationRate ?? FINANCIAL_DEFAULTS.electricityEscalationRate;
  ...
  const batteryDegradation =
    inputs.batteryDegradationRate ?? FINANCIAL_DEFAULTS.batteryDegradationRate;
  ...
  for (let year = 1; year <= years; year++) {
    ...
    const degradationFactor = hasSolar ? batteryFactor * 0.6 + solarFactor * 0.4 : batteryFactor;
  ...
  const irr = calculateIRR(
    inputs.netInvestment,
    yearlyProjections.map((y) => y.cashFlow)
  );
