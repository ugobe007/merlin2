...
export function calculatePowerProfile(wizardData: {
  ...
  POWER_PROFILE_CHECKS.forEach((check) => {
    ...
    switch (check.id) {
      ...
      case "building_size":
        completed = !!(
          wizardData.useCaseData?.squareFootage || wizardData.useCaseData?.facilitySize
        );
        break;
      ...
      case "utility_bill_data":
        completed = !!(
          wizardData.useCaseData?.monthlyElectricBill || wizardData.useCaseData?.peakDemandKw
        );
        break;
      case "grid_quality":
      ...
      case "existing_solar":
        completed = !!(
          wizardData.useCaseData?.existingSolarKw && wizardData.useCaseData.existingSolarKw > 0
        );
        break;
      case "existing_ev":
        completed = !!(
          wizardData.useCaseData?.existingEvPorts && wizardData.useCaseData.existingEvPorts > 0
        );
        break;
      ...
      case "generator_backup":
        completed = !!(
          wizardData.useCaseData?.generatorSizeKw && wizardData.useCaseData.generatorSizeKw > 0
        );
        break;
