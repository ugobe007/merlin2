...
import { calculateQuote, estimatePayback } from './unifiedQuoteCalculator';
import { calculateFinancialMetrics } from './centralizedCalculations';
import { getGeographicRecommendations, getRegionalElectricityRate } from './geographicIntelligenceService';
...
export async function getCalculationConstants(): Promise<CalculationConstants> {
  ...
  try {
    ...
    data.forEach((row: { key: string; value_numeric: number }) => {
      switch (row.key) {
        case 'battery_cost_per_kwh_small': constants.batteryCostPerKwhSmall = row.value_numeric; break;
        case 'battery_cost_per_kwh_medium': constants.batteryCostPerKwhMedium = row.value_numeric; break;
        case 'battery_cost_per_kwh_large': constants.batteryCostPerKwhLarge = row.value_numeric; break;
        case 'solar_cost_per_watt': constants.solarCostPerWatt = row.value_numeric; break;
        case 'inverter_cost_per_kw': constants.inverterCostPerKw = row.value_numeric; break;
        case 'installation_percentage': constants.installationPercentage = row.value_numeric; break;
        case 'federal_itc_rate': constants.federalItcRate = row.value_numeric; break;
        case 'discount_rate': constants.discountRate = row.value_numeric; break;
        case 'project_lifetime_years': constants.projectLifetimeYears = row.value_numeric; break;
        case 'battery_degradation_rate': constants.batteryDegradationRate = row.value_numeric; break;
        case 'electricity_escalation_rate': constants.electricityEscalationRate = row.value_numeric; break;
        case 'peak_shaving_target_percent': constants.peakShavingTargetPercent = row.value_numeric; break;
        case 'backup_hours_minimum': constants.backupHoursMinimum = row.value_numeric; break;
        case 'backup_hours_recommended': constants.backupHoursRecommended = row.value_numeric; break;
        case 'solar_to_storage_ratio': constants.solarToStorageRatio = row.value_numeric; break;
      }
    });
 ...
 */
export async function getIndustryPowerProfile(industrySlug: string): Promise<IndustryPowerProfile | null> {
  if (!isSupabaseConfigured()) {
    return null;
  }
...
export async function generateSMBQuote(input: SMBQuoteInput) {
  ...
  const peakDemandKw = input.peakDemandKw || 
    (industryProfile ? industryProfile.typicalPeakDemandKw * (input.unitCount / 5) : 100 * input.unitCount);
  
  const monthlyKwh = input.monthlyKwh ||
    (industryProfile ? industryProfile.typicalMonthlyKwh * (input.unitCount / 5) : 5000 * input.unitCount);
 ...
 */
export async function saveSMBLead(lead: SMBLead): Promise<{ success: boolean; id?: string; error?: string }> {
  if (!isSupabaseConfigured()) {
