...
function getMarketBenchmarks(): Record<string, { min: number; max: number; avg: number }> {
  return {
    ...
    bos: {
      min: 0.10, // 10% of equipment cost
      max: 0.30, // 30% of equipment cost
      avg: 0.20, // 20% of equipment cost
    },
    container: {
      min: 500,  // $/unit
      max: 2000, // $/unit
 ...
 */
export async function validateVendorPricing(
  product: {
  ...
  if (pricePerUnit && (category === 'battery' || category === 'inverter' || category === 'ems')) {
    try {
      ...
      if (isSupabaseConfigured()) {
        ...
        const equipmentTypeMap: Record<string, 'bess' | 'inverter' | 'solar' | 'wind' | 'generator' | 'ev-charger'> = {
          battery: 'bess',
          inverter: 'inverter',
        ...
        const marketSummary = await getMarketPriceSummary(
          equipmentType,
          'north-america'
        );
      ...
      if (deviationAbs > 20) {
        if (pricePerUnit < marketMin! * 0.8) {
          ...
          score -= 0.15;
          recommendations.push('Verify pricing is correct. Very low prices may indicate data entry error.');
 ...
 */
export async function validateVendorProductById(productId: string): Promise<VendorProductValidation> {
  if (!isSupabaseConfigured()) {
