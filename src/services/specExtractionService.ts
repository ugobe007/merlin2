...
function extractWithRegex(text: string, rawMatches: string[]): ExtractedSpecs {
  ...
  // kWh patterns
  if (!specs.energyMWh) {
    const kwhPatterns = [/(\d+(?:\.\d+)?)\s*kwh\b/gi, /energy\s*[:=]?\s*(\d+(?:\.\d+)?)\s*kwh/gi];
  ...
  for (const [abbr, name] of Object.entries(US_STATES)) {
    const patterns = [new RegExp(`\\b${abbr}\\b`, "g"), new RegExp(`\\b${name}\\b`, "gi")];
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a technical document analyzer specializing in energy storage systems. Extract specifications accurately and return valid JSON only.",
          },
 ...
 */
function validateSpecs(specs: ExtractedSpecs, warnings: string[], suggestions: string[]): void {
  ...
  if (!specs.industry) {
    suggestions.push(
      "Specify industry (hotel, hospital, data center, etc.) for optimized recommendations"
    );
  }
...
function generateSummary(specs: ExtractedSpecs): string {
  ...
  if (specs.storageSizeMW) {
    const energy =
      specs.energyMWh || (specs.durationHours ? specs.storageSizeMW * specs.durationHours : null);
