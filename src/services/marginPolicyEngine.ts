...
export const DEFAULT_MARGIN_BANDS: MarginBand[] = [
  ...
  },
  {
    ...
    marginMax: 0.05,
    marginTarget: 0.035,
    description: "Enterprise: $20M-$100M",
  },
  {
    id: "mega",
    minTotal: 100_000_000,
    maxTotal: null,
    marginMin: 0.005,
    marginMax: 0.02,
    marginTarget: 0.012,
    description: "Mega: $100M+",
  },
];
...
export const DEFAULT_PRODUCT_MARGINS: ProductMarginConfig[] = [
  {
    ...
    isAdditive: false,
    description: "BESS: Standard margin",
  },
  {
    productClass: "solar",
    marginMultiplier: 0.75,
    isAdditive: false,
    description: "Solar: Commoditized, tighter margin",
  },
  {
    productClass: "wind",
    marginMultiplier: 0.85,
    isAdditive: false,
    description: "Wind: Slightly tighter margin",
  },
  {
    productClass: "generator",
    marginMultiplier: 0.9,
    isAdditive: false,
    description: "Generator: Competitive market",
  },
  {
    productClass: "ev_charger",
    marginMultiplier: 1.1,
    isAdditive: false,
    description: "EV: Higher margin (install complexity)",
  },
  {
    productClass: "inverter_pcs",
    marginMultiplier: 0.85,
    isAdditive: false,
    description: "Inverter: Tight margin, stable",
  },
  {
    productClass: "transformer",
    marginMultiplier: 0.9,
    isAdditive: false,
    description: "Transformer: Moderate margin",
  },
  {
    productClass: "microgrid_controller",
    marginMultiplier: 1.2,
    isAdditive: false,
    description: "Microgrid: Higher complexity margin",
  },
  {
    productClass: "bms",
    marginMultiplier: 1.0,
    isAdditive: false,
    description: "BMS: Standard margin",
  },
  {
    productClass: "scada",
    marginMultiplier: 1.15,
    isAdditive: false,
    description: "SCADA: Software premium",
  },
  {
    productClass: "ems_software",
    marginMultiplier: 1.25,
    isAdditive: false,
    description: "EMS: Software premium",
  },
  {
    productClass: "construction_labor",
    marginMultiplier: 1.0,
    isAdditive: true,
    fixedAdder: 0.15,
    description: "Labor: 15% fixed adder",
  },
  {
    ...
    isAdditive: true,
    fixedAdder: 0.2,
    description: "Engineering: 20% fixed adder",
  },
];
...
export const DEFAULT_RISK_ADJUSTERS: RiskAdjuster[] = [
  ...
  {
    riskLevel: "elevated",
    marginAddPercent: 0.02,
    description: "Elevated: Data center, critical loads",
  },
  {
    riskLevel: "high_complexity",
    marginAddPercent: 0.04,
    description: "High: Microgrid islanding, N+1, hospital",
  },
];
...
export const DEFAULT_SEGMENT_ADJUSTERS: SegmentAdjuster[] = [
  ...
  { segment: "strategic", marginMultiplier: 0.9, description: "Strategic account: 10% discount" },
  { segment: "government", marginMultiplier: 0.8, description: "Government: 20% discount" },
];
...
export const DEFAULT_PRICE_GUARDS: PriceGuard[] = [
  ...
  },
  {
    ...
    asOfDate: "2025-12-01",
    procurementBufferPct: 0.1,
    procurementBufferTrigger: 0.7,
...
export const DEFAULT_QUOTE_GUARDS: QuoteLevelGuard[] = [
  ...
  { productClass: "solar", metric: "per_W_total", minPerUnit: 0.8, maxPerUnit: 2.5 }, // Total solar project
];
 ...
 */
export function getMarginBand(
  totalBaseCost: number,
  bands: MarginBand[] = DEFAULT_MARGIN_BANDS
): MarginBand {
  for (const band of bands) {
    if (
      totalBaseCost >= band.minTotal &&
      (band.maxTotal === null || totalBaseCost < band.maxTotal)
    ) {
      return band;
    }
  }
 ...
 */
export function getProductMarginMultiplier(
  productClass: ProductClass,
  configs: ProductMarginConfig[] = DEFAULT_PRODUCT_MARGINS
): ProductMarginConfig {
  return (
    configs.find((c) => c.productClass === productClass) || {
      productClass,
      marginMultiplier: 1.0,
      isAdditive: false,
      description: "Default",
 ...
 * Get risk adjuster
 */
export function getRiskAdjuster(
  riskLevel: RiskLevel,
  adjusters: RiskAdjuster[] = DEFAULT_RISK_ADJUSTERS
): RiskAdjuster {
 ...
 */
export function getSegmentAdjuster(
  segment: CustomerSegment,
  adjusters: SegmentAdjuster[] = DEFAULT_SEGMENT_ADJUSTERS
): SegmentAdjuster {
 ...
 */
export function getPriceGuard(
  productClass: ProductClass,
  guards: PriceGuard[] = DEFAULT_PRICE_GUARDS
): PriceGuard | undefined {
...
export function applyMarginToLineItem(
  ...
  const baseMargin = marginBand.marginTarget;
  ...
  const riskAdjustedMargin = productAdjustedMargin + riskAdjuster.marginAddPercent;
  ...
  if (guard && item.quantity > 0) {
    ...
    if (sellUnitPrice > guard.ceilingPrice) {
      ...
      clampEvents.push({
        ...
        guardName:
          clampedPerUnit > guard.ceilingPrice
  ...
  return {
    ...
    // Clamping & review
    wasClampedFloor: clampEvents.some(
      (e) => e.reason === "unit_floor" || e.reason === "margin_floor"
    ),
    wasClampedCeiling: clampEvents.some(
      (e) => e.reason === "unit_ceiling" || e.reason === "margin_ceiling"
    ),
    clampEvents,
    reviewEvents,
...
export function applyMarginPolicy(input: MarginPolicyInput): MarginQuoteResult {
  ...
  const blendedMarginPercent =
    obtainableCostTotal > 0 ? totalMarginDollars / obtainableCostTotal : 0;
  ...
  if (blendedMarginPercent < 0.02) {
    warnings.push(
      `⚠️ Blended margin (${(blendedMarginPercent * 100).toFixed(1)}%) below 2% minimum`
    );
    passesGuards = false;
  }
  if (blendedMarginPercent > 0.3) {
    warnings.push(
      `⚠️ Blended margin (${(blendedMarginPercent * 100).toFixed(1)}%) above 30% maximum`
    );
    passesGuards = false;
  }
  ...
  for (const guard of DEFAULT_QUOTE_GUARDS) {
    ...
    if (perUnit < guard.minPerUnit) {
      warnings.push(
        `⚠️ ${guard.productClass} quote ${guard.metric} below floor: $${perUnit.toFixed(2)} < $${guard.minPerUnit}`
      );
      passesGuards = false;
    }
    if (perUnit > guard.maxPerUnit) {
      warnings.push(
        `⚠️ ${guard.productClass} quote ${guard.metric} above ceiling: $${perUnit.toFixed(2)} > $${guard.maxPerUnit}`
      );
      passesGuards = false;
    }
  }
  ...
  if (sellPriceTotal < obtainableCostTotal) {
    warnings.push(
      `🚨 CRITICAL: Negative gross margin detected! sellPrice ($${sellPriceTotal.toFixed(2)}) < obtainableCost ($${obtainableCostTotal.toFixed(2)})`
    );
    passesGuards = false;
  }
  ...
  if (allReviewEvents.length > 0) {
    warnings.push(
      `⚠️ ${allReviewEvents.length} item(s) need human review (market price below threshold)`
    );
  }
...
export async function applyMarginPolicyAsync(input: MarginPolicyInput): Promise<MarginQuoteResult> {
  ...
  const marginBand =
    marginBands.find(
      (b) =>
        input.totalBaseCost >= b.minTotal &&
        (b.maxTotal === null || input.totalBaseCost < b.maxTotal)
...
export function estimateMargin(
  baseCost: number,
  productClass: ProductClass = "bess"
): {
