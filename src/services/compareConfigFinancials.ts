...
export function calculateFinancialsSync(inputs: FinancialInputs): FinancialResult {
  ...
  // ITC incentive (30%)
  const incentives = grossCost * 0.3;
...
function formatQuoteResult(
  ...
  const grossCost =
    costs.totalProjectCost || batteryCost + solarCost + windCost + generatorCost + installationCost;
  const incentives = costs.taxCredit || grossCost * 0.3;
  ...
  const roi25Year =
    financials.roi25Year || ((annualSavings * 25 - netInvestment) / netInvestment) * 100;
...
export function compareConfigurations(
  ...
  const investmentDiffPercent =
    merlin.netInvestment > 0 ? (investmentDiff / merlin.netInvestment) * 100 : 0;

  const savingsDiff = user.annualSavings - merlin.annualSavings;
  const savingsDiffPercent =
    merlin.annualSavings > 0 ? (savingsDiff / merlin.annualSavings) * 100 : 0;
