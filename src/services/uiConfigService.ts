...
const DEFAULT_CAR_WASH_LIMITS: CarWashUILimits = {
  ...
  standardBlowers: { min: 0, max: 20, default: 6, step: 1, powerKW: 7.5, label: 'Standard Blowers' },
  vacuumStations: { min: 0, max: 40, default: 8, step: 1, powerKW: 3, label: 'Vacuum Stations' },
  highPressurePumps: { min: 0, max: 8, default: 2, step: 1, powerKW: 11, label: 'High-Pressure Pump Stations' },
  topBrushes: { min: 0, max: 6, default: 2, step: 1, powerKW: 4, label: 'Top Brushes' },
  wrapAroundBrushes: { min: 0, max: 8, default: 4, step: 1, powerKW: 3.7, label: 'Wrap-Around Brushes' },
  ...
  chemicalStations: { min: 0, max: 8, default: 4, step: 1, powerKW: 1.5, label: 'Chemical Pump Stations' },
  airCompressorHP: { min: 5, max: 50, default: 10, step: 5, label: 'Air Compressor (HP)' },
  solarRoofArea: { min: 0, max: 50000, default: 5000, step: 500, unit: 'sqft', label: 'Available Roof Area for Solar' },
  ...
  targetSavingsPercent: { min: 10, max: 80, default: 40, step: 5, label: 'Target Demand Reduction %' },
  currentMonthlyBill: { min: 500, max: 50000, default: 5000, step: 100, label: 'Current Monthly Electric Bill' },
};
...
const DEFAULT_BESS_LIMITS: BESSUILimits = {
  ...
  electricityRate: { min: 0.05, max: 0.50, default: 0.12, step: 0.01, unit: '$/kWh', label: 'Electricity Rate' },
  demandChargeRate: { min: 5, max: 50, default: 15, step: 1, unit: '$/kW', label: 'Demand Charge Rate' },
 ...
 */
export function useUILimits<T = Record<string, UIFieldLimit>>(configKey: string): {
  limits: T | null;
  loading: boolean;
  error: Error | null;
} {
