...
const STATE_PROFILES: Record<string, GeographicProfile> = {
  ...
  CA: {
    ...
    considerations: [
      "High electricity rates make storage very attractive",
      "Fire season backup critical",
    ],
  },
  ...
  AZ: {
    ...
    recommendedTechnologies: ["solar", "battery"],
    primaryBenefits: [
      "Best solar in the country",
      "Extreme summer AC load shifting",
      "Demand charge reduction",
    ],
    considerations: [
      "Extreme heat requires proper equipment ratings",
      "High cooling loads in summer",
    ],
  },
  ...
  NV: {
    ...
    considerations: [
      "Water scarcity for some cooling systems",
  ...
  TX: {
    ...
    primaryBenefits: [
      "Grid independence critical",
      "High solar + wind potential",
      "Wholesale price volatility hedge",
    ],
    considerations: [
      "ERCOT grid vulnerability",
      "Winter storm backup essential",
      "No net metering in most areas",
    ],
  },
  ...
  NY: {
    ...
    considerations: [
      ...
      "Heavy winter heating loads",
      "Complex permitting in NYC",
    ],
  },
  ...
  WA: {
    ...
    considerations: [
      "Low electricity rates reduce ROI",
      "Limited solar benefit",
      "Focus on resilience over savings",
    ],
  },
...
export function getSmartRecommendations(
  ...
  const batteryDurationHours =
    profile.gridReliability === "poor" || profile.gridReliability === "fair"
      ? Math.max(4, backupHours)
      : backupHours;
  const batteryReason =
    profile.gridReliability === "excellent"
  ...
  // Summary
  const summary =
    `Based on ${profile.state}'s ${profile.solarRating} solar potential, ` +
...
export function getElectricityRate(
  stateCode: string,
  type: "residential" | "commercial" | "industrial" = "commercial"
): number {
  const profile = getStateProfile(stateCode);
  switch (type) {
    case "residential":
      return profile.averageResidentialRate;
    case "commercial":
      return profile.averageCommercialRate;
    case "industrial":
      return profile.averageIndustrialRate;
  }
}
...
export function getGeographicRecommendations(stateOrCode: string): GeographicRecommendation {
  ...
  const simplifiedProfile: StateEnergyProfile = {
    ...
    gridReliabilityScore:
      fullProfile.gridReliability === "excellent"
        ? 95
        : fullProfile.gridReliability === "good"
          ? 80
          : fullProfile.gridReliability === "fair"
            ? 60
            : 40,
    avgDemandCharge: fullProfile.demandChargeAvg,
    windCapacityFactor: ["TX", "OK", "KS", "NE", "IA", "MN", "SD", "ND"].includes(stateCode || "")
      ? 0.35
      : 0.15,
  ...
  const solarRecommended =
    fullProfile.solarRating === "excellent" || fullProfile.solarRating === "good";
  const batteryRecommended = true; // Almost always recommended
  const windRecommended = ["TX", "OK", "KS", "NE", "IA", "MN", "SD", "ND"].includes(
    stateCode || ""
  );
  const generatorRecommended =
    fullProfile.gridReliability === "fair" || fullProfile.gridReliability === "poor";
...
export function getRecommendedAddOns(stateOrCode: string): string[] {
  ...
  if (
    ["CA", "WA", "OR", "CO", "NY", "MA", "NJ"].includes(stateOrCode.length === 2 ? stateOrCode : "")
  ) {
