 ...
 *
 * PLATFORM IMPORT (April 2026):
 *   applyPlatformDesign() accepts a SolarDesignImport from any supported
 *   third-party design tool (Aurora Solar, HelioScope, OpenSolar, etc.) and
 *   returns a SolarCapacityResult that uses the platform's actual sizing
 *   instead of Merlin's calculated sizing.
 */
...
import type { SolarDesignImport } from "./solarSizingIntegrationService";
import { PLATFORM_DISPLAY_NAMES, designToQuoteOverride } from "./solarSizingIntegrationService";
...
// ============================================================================
// PLATFORM DESIGN IMPORT
// ============================================================================

/**
 * Apply an imported solar design from a third-party sizing platform.
 *
 * Overrides Merlin's calculated solar capacity with data from Aurora Solar,
 * HelioScope, OpenSolar, Solargraf, EagleView TrueDesign, Scanifly, PV*SOL,
 * or SolarAPP+ when a contractor has already created a precise site design.
 *
 * The platform's system size and production data take priority.
 * Merlin's template factors are used as fallback for any missing fields.
 *
 * @param design   - Normalized SolarDesignImport from solarSizingIntegrationService
 * @param industry - Merlin industry string (for template-based fallbacks)
 * @returns SolarCapacityResult using platform data, fully audit-trailed
 */
export function applyPlatformDesign(
  design: SolarDesignImport,
  industry: string
): SolarCapacityResult {
  const template = getSolarTemplate(industry);
  const override = designToQuoteOverride(design);
  const platformName = PLATFORM_DISPLAY_NAMES[design.platform];
  const now = new Date().toISOString();

  const systemKw = override.importedSystemKw;

  // Derive roof area: use platform's actual covered area, or back-calculate from kW
  const roofAreaSqFt =
    (override.importedRoofAreaSqFt ?? systemKw * 1000 * template.solarDensity > 0)
      ? (systemKw * 1000) / (template.solarDensity * 1000)
      : 0;
  ...
  // Split total kW into roof vs carport based on template ratio
  // (platform doesn't always distinguish, so we attribute all to roof)
  const roofSolarKW = design.isCarport ? 0 : systemKw;
  const carportSolarKW = design.isCarport ? systemKw : 0;

  return {
    roofArea: roofAreaSqFt,
    roofUnit: "sqft",
    carportArea: design.isCarport ? roofAreaSqFt : 0,
    carportUnit: "sqft",
    carportInterest: design.isCarport ? "yes" : "no",

    roofSolarUsable: roofAreaSqFt,
    roofSolarKW,
    roofUsableFactor: template.roofUsableFactor,

    carportSolarUsable: design.isCarport ? roofAreaSqFt : 0,
    carportSolarKW,
    carportUsableFactor: template.carportUsableFactor,

    totalSolarArea: roofAreaSqFt,
    totalSolarKW: systemKw,
    systemSizeCategory,
    annualGenerationKWh,

    industryTemplate: industry,
    templateVersion: `platform-import:${design.platform}`,

    calculations: {
      roofUsable: {
        formula: `Imported from ${platformName}`,
        inputs: {
          platform: design.platform,
          platformProjectId: design.platformProjectId ?? "",
          systemSizeDC_kW: systemKw,
        },
        result: roofSolarKW,
        unit: "kW",
        notes: `Platform import — Merlin calculation overridden by ${platformName} design`,
      },
      roofGeneration: {
        formula: override.importedAnnualKwh
          ? `${platformName} simulation result`
          : `${systemKw} kW × template production factor`,
        inputs: { systemKw, annualKwh: annualGenerationKWh },
        result: annualGenerationKWh,
        unit: "kWh/yr",
        notes: override.importedAnnualKwh
          ? `Year-1 production from ${platformName}`
          : "Estimated using Merlin template (platform did not provide production data)",
      },
      totalArea: {
        formula: "From platform design",
        inputs: { roofAreaSqFt },
        result: roofAreaSqFt,
        unit: "sqft",
      },
      totalGeneration: {
        formula: "Platform total",
        inputs: { systemKw },
        result: systemKw,
        unit: "kW",
      },
      annualGeneration: {
        formula: "Platform or Merlin estimate",
        inputs: { annualGenerationKWh },
        result: annualGenerationKWh,
        unit: "kWh",
      },
    },
    ...
    calculatedAt: now,
    engineVersion: `TrueQuoteEngine v2.1.0 + ${platformName} Import`,
  };
}
