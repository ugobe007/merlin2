...
const MACRS_5_YEAR = [0.2, 0.32, 0.192, 0.1152, 0.1152, 0.0576];
 ...
 */
function calculateCapacityPayments(powerMW: number, isoRegion: string): number {
...
function calculateResourceAdequacy(
  ...
  const isCA =
    location.toLowerCase().includes("california") || location.toLowerCase().includes("ca");
...
export async function generateProfessionalModel(
  ...
  if (import.meta.env.DEV) {
    console.log(
      `💵 Debt: $${(debtAmount / 1000000).toFixed(2)}M | Equity: $${(equityAmount / 1000000).toFixed(2)}M`
    );
  }
  ...
  for (let year = 1; year <= config.projectLifeYears; year++) {
    ...
    if (input.revenueStreams.frequencyRegulation) {
      frequencyReg =
        calculateFrequencyRegulationRevenue(
          input.storageSizeMW * degradationFactor,
          config.isoRegion,
    ...
    if (input.revenueStreams.spinningReserve) {
      spinningReserve =
        calculateSpinningReserveRevenue(
          input.storageSizeMW * degradationFactor,
          config.isoRegion,
          8
        ) * escalationFactor;
    ...
    if (input.revenueStreams.capacityPayments) {
      capacity =
        calculateCapacityPayments(input.storageSizeMW * degradationFactor, config.isoRegion) *
        escalationFactor;
    }
  ...
  for (let year = 1; year <= config.projectLifeYears; year++) {
    ...
    balanceSheets.push({
      ...
      totalAssets: Math.round(
        cf.cumulativeCashFlow + equityAmount + totalCapex - deps.accumulatedDepreciation
      ),
  ...
  const avgDSCR =
    dscrValues.length > 0 ? dscrValues.reduce((a, b) => a + b, 0) / dscrValues.length : 0;
  ...
  const annualOpex =
    capacityKWh * config.omCostPerKWhYear + totalCapex * config.insurancePercentCapex;
  ...
  const keyMetrics = revenueProjection.map((rev, idx) => ({
    ...
    cumulativeROI: Math.round(
      ((cashFlowStatements[idx].cumulativeCashFlow + equityAmount) / equityAmount - 1) * 100
    ),
  }));
  ...
  if (import.meta.env.DEV) {
    console.log(
      `📈 Unlevered IRR: ${unleveredIRR.toFixed(1)}% | Levered IRR: ${leveredIRR.toFixed(1)}%`
    );
  }
  if (import.meta.env.DEV) {
    console.log(
      `📊 NPV: $${(npv / 1000000).toFixed(2)}M | Min DSCR: ${minDSCR.toFixed(2)}x | LCOS: $${lcos}/MWh`
    );
  }
    ...
    for (const variation of variations) {
      ...
      switch (param) {
        ...
        case "interestRate":
          ...
          break;
        case "degradation":
          modifiedInput.annualDegradation =
            (modifiedInput.annualDegradation ?? 0.02) * (1 + variation);
