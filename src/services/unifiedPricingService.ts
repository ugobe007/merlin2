 ...
 * 3. NREL ATB 2024/2025 - National Renewable Energy Laboratory Annual Technology Baseline
 *    (Note: Checking for 2025/2026 data availability)
 *
...
import { getConstant, clearConstantsCache } from "./calculationConstantsService";
import { getMarketAdjustedPrice, clearMarketDataCache } from "./marketDataIntegrationService";
import { getVendorPricing } from "./vendorPricingIntegrationService";
import type {
  BatteryPricing,
  InverterPricing,
  TransformerPricing,
...
export function clearAllPricingCaches(): void {
  ...
  if (import.meta.env.DEV) {
    console.log("🔄 All pricing caches cleared - will fetch fresh from database + market data");
  }
}

// ============================================
// NREL ATB FALLBACK DATA (2024/2025)
// ============================================
// Note: NREL ATB 2025 may be available - using 2024 as fallback
...
const NREL_WIND_PRICING: WindPricing = {
  pricePerKW: 1200, // $1,200/kW for land-based wind (NREL ATB 2024, may be updated)
  manufacturer: "Various (NREL ATB 2024)",
 ...
 * 5. NREL ATB 2024/2025 fallback constants
 */
async function fetchBatteryPricingFromDB(
  ...
  try {
    ...
    if (vendorPricing && vendorPricing.pricePerKwh) {
      if (import.meta.env.DEV) {
        console.log(
          `🏢 Battery pricing from VENDOR: $${vendorPricing.pricePerKwh}/kWh (${vendorPricing.manufacturer} ${vendorPricing.model})`
        );
      }
    ...
    if (pricePerKWh !== null) {
      if (import.meta.env.DEV) {
        console.log(
          `💾 Battery pricing from calculation_constants: $${pricePerKWh}/kWh (${energyKWh.toFixed(0)} kWh system)`
        );
      }
      return {
        pricePerKWh,
        manufacturer:
          energyKWh >= 10000 ? "CATL/BYD" : energyKWh >= 1000 ? "Great Power" : "LiON Energy",
        model:
          energyKWh >= 10000
            ? "Utility Scale LFP"
            : energyKWh >= 1000
    ...
    if (!error && data) {
      ...
      if (import.meta.env.DEV) {
        console.log(
          `💾 Battery pricing from equipment_pricing: $${vendorPrice.toFixed(0)}/kWh (${data.manufacturer})`
        );
      }
    ...
    const marketData = await getMarketAdjustedPrice(
      "bess",
      NREL_ATB_2024_BATTERY.pricePerKWh,
      location
    );
    ...
    if (marketData.source === "market" && marketData.confidence !== "low") {
      if (import.meta.env.DEV) {
        console.log(
          `📈 Battery pricing from MARKET DATA: $${marketData.price}/kWh (${marketData.confidence} confidence, ${marketData.dataPoints || 0} data points)`
        );
    ...
    if (import.meta.env.DEV) {
      console.log(
        `📦 Battery pricing from NREL fallback: $${NREL_ATB_2024_BATTERY.pricePerKWh}/kWh`
      );
    }
...
export function getCacheStatus(): {
  ...
  const cachedItems = Object.entries(pricingCache)
    .filter(
      ([key, value]) => key !== "lastCacheUpdate" && key !== "cacheExpiryMinutes" && value !== null
    )
    .map(([key]) => key);
