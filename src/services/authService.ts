...
function writeCache(user: User | null): void {
  if (!user) { localStorage.removeItem(CURRENT_USER_KEY); return; }
...
function rowToUser(row: Record<string, any>): User {
  return {
    ...
    preferences: row.preferences ?? { defaultCurrency: 'USD', defaultLocation: 'United States', emailNotifications: true },
  };
}
...
class SupabaseAuthService {
  constructor() {
    supabase.auth.onAuthStateChange(async (_event, session) => {
      if (!session?.user) { writeCache(null); return; }
      try {
      ...
      } catch { writeCache(this._metaToUser(session.user)); }
    });
  }
  ...
  private _metaToUser(authUser: any): User {
    ...
    return {
      ...
      preferences: { defaultCurrency: 'USD', defaultLocation: 'United States', emailNotifications: true },
    };
  }
  ...
  async signUp(
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        email, password,
        options: { data: { first_name: firstName, last_name: lastName, company, account_type: accountType } },
      });
      ...
      const profileRow = {
        ...
        company: company ?? null, tier: 'free', account_type: accountType,
  ...
  async signIn(email: string, password: string): Promise<AuthResponse> {
    try {
      ...
      const { data: pd } = await supabase.from('user_profiles').select('*').eq('id', data.user.id).single();
  ...
  getCurrentUser(): User | null { return readCache(); }
  ...
  async updateUserProfile(userId: string, updates: Partial<User>): Promise<AuthResponse> {
    try {
      ...
      if (updates.publicProfileSlug !== undefined) db.public_profile_slug = updates.publicProfileSlug;
      if (updates.profileVisibility !== undefined) db.profile_visibility = updates.profileVisibility;
      ...
      const { data, error } = await supabase.from('user_profiles').update(db).eq('id', userId).select().single();
      if (error) return { success: false, error: error.message };
      const user = rowToUser(data);
      if (readCache()?.id === userId) writeCache(user);
      return { success: true, user };
    } catch { return { success: false, error: 'Failed to update profile' }; }
  }

  async deleteAccount(_email: string): Promise<boolean> {
    try { await this.signOut(); return true; } catch { return false; }
  }
  ...
  async resetPassword(email: string, _newPassword?: string): Promise<AuthResponse> {
    ...
    } catch { return { success: false, error: 'Password reset failed' }; }
  }
  ...
  async getPublicProfile(slug: string): Promise<User | null> {
    try {
      const { data } = await supabase.from('user_profiles').select('*')
        .eq('public_profile_slug', slug).eq('profile_visibility', 'public').single();
      if (!data) return null;
  ...
  getCompanyById(_id: string): Company | null { return null; }
  getCompanyMembers(_companyId: string): User[] { return []; }
  async inviteTeamMember(_c: string, _e: string) { return { success: false, error: 'Team features coming soon', inviteCode: undefined as string | undefined }; }
  async joinCompanyWithInvite(_u: string, _i: string): Promise<AuthResponse> { return { success: false, error: 'Team features coming soon' }; }
  getAllAccounts() { const u = readCache(); return u ? [{ email: u.email, firstName: u.firstName, lastName: u.lastName }] : []; }
}
...
if (typeof window !== 'undefined' && import.meta.env.DEV) {
  (window as any).authDebug = {
    ...
    clearCache: () => { localStorage.removeItem('current_user'); console.log('Cache cleared'); },
