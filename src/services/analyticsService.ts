...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [pvAll, pvToday, pvSessions, pvRaw] = await Promise.all([
    q(db.from("page_views").select("id", { count: "exact", head: true }).gte("created_at", ago30), EMPTY_COUNT),
    q(db.from("page_views").select("id", { count: "exact", head: true }).gte("created_at", `${today}T00:00:00`), EMPTY_COUNT),
  ...
  const [quotesAll, quotesToday, quotesRaw] = await Promise.all([
    q(supabase.from("saved_quotes").select("id", { count: "exact", head: true }).gte("created_at", ago30), EMPTY_COUNT),
    q(supabase.from("saved_quotes").select("id", { count: "exact", head: true }).gte("created_at", `${today}T00:00:00`), EMPTY_COUNT),
    q(supabase.from("saved_quotes").select("created_at, use_case").gte("created_at", ago30), EMPTY_DATA),
  ]);
  ...
  const topIndustries: TopItem[] = Object.entries(industryCount)
    .sort((a, b) => b[1] - a[1]).slice(0, 6).map(([label, count]) => ({ label, count }));

  // ── users ────────────────────────────────────────────────────────────────
  const [signupsAll, signupsToday, signupsRaw, recentSignupsRaw] = await Promise.all([
    q(db.from("users").select("id", { count: "exact", head: true }).gte("created_at", ago30), EMPTY_COUNT),
    q(db.from("users").select("id", { count: "exact", head: true }).gte("created_at", `${today}T00:00:00`), EMPTY_COUNT),
    q(db.from("users").select("created_at").gte("created_at", ago30), EMPTY_DATA),
    q(db.from("users").select("email, created_at").order("created_at", { ascending: false }).limit(8), EMPTY_DATA),
  ]);
  ...
  const [leadsAll, leadsToday, recentLeadsRaw] = await Promise.all([
    q(supabase.from("leads").select("id", { count: "exact", head: true }).gte("created_at", ago30), EMPTY_COUNT),
    q(supabase.from("leads").select("id", { count: "exact", head: true }).gte("created_at", `${today}T00:00:00`), EMPTY_COUNT),
    q(supabase.from("leads").select("email, industry, created_at").order("created_at", { ascending: false }).limit(8), EMPTY_DATA),
  ]);
  ...
  return {
    ...
    recentLeads:   (recentLeadsRaw.data   ?? []) as { email: string; industry: string | null; created_at: string }[],
  };
}
