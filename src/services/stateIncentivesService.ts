...
const ZIP_TO_STATE: { [key: string]: string } = {
  ...
  '025': 'MA', '026': 'MA', '027': 'RI', '028': 'RI', '029': 'RI',
...
export async function getStateSummary(stateCode: string): Promise<StateIncentiveSummary | null> {
  ...
  const taxCredits = incentives.filter(i => i.program_type === 'tax_credit' && i.incentive_percent);
...
const FALLBACK_INCENTIVES: { [key: string]: Partial<StateIncentive>[] } = {
  'CA': [
    ...
    { program_name: 'SGIP - Equity', incentive_per_kwh: 850, program_type: 'rebate', equity_program: true }
  ],
  'NY': [
    { program_name: 'NYSERDA Retail Storage', incentive_per_kwh: 175, program_type: 'rebate' }
  ],
  'MA': [
    { program_name: 'ConnectedSolutions', incentive_per_kwh: 225, program_type: 'rebate' }
  ],
  'NV': [
    { program_name: 'Property Tax Exemption', incentive_percent: 100, program_type: 'property_tax_exemption' }
  ],
  'TX': [
    { program_name: 'Property Tax Exemption', incentive_percent: 100, program_type: 'property_tax_exemption' }
  ],
  'MD': [
    { program_name: 'Energy Storage Tax Credit', incentive_percent: 30, max_incentive: 5000, program_type: 'tax_credit' }
  ],
  'AZ': [
    { program_name: 'Storage Tax Credit', incentive_percent: 25, max_incentive: 1000, program_type: 'tax_credit' }
  ],
  'CO': [
    { program_name: 'Storage Tax Credit', incentive_percent: 10, program_type: 'tax_credit' }
  ]
};
