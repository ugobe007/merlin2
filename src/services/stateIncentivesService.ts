...
export interface StateIncentive {
  ...
  state_name: string;
  program_name: string;
  program_type: "rebate" | "tax_credit" | "grant" | "property_tax_exemption";
  ...
  requires_solar_pairing: boolean;
  income_restricted: boolean;
  equity_program: boolean;
  equity_incentive_per_kwh: number | null;
  fire_zone_eligible: boolean;
  is_active: boolean;
...
export interface StateIncentiveSummary {
  state_code: string;
  state_name: string;
  total_programs: number;
  best_rebate_per_kwh: number;
  best_tax_credit_percent: number;
  has_equity_programs: boolean;
  has_property_tax_exemption: boolean;
  estimated_incentive: number; // Calculated based on system size
}
...
const ZIP_TO_STATE: { [key: string]: string } = {
  // First 3 digits of ZIP code to state
  "005": "NY",
  "006": "PR",
  "007": "PR",
  "008": "VI",
...
/**
 * Fetch all active incentives for a state
 */
export async function getStateIncentives(stateCode: string): Promise<StateIncentive[]> {
  try {
    const { data, error } = await supabase
      ...
      .eq("is_active", true)
      .order("incentive_per_kwh", { ascending: false, nullsFirst: false });

    if (error) {
      console.warn("⚠️ State incentives query failed:", error);
      return [];
    }
...
/**
 * Fetch incentives by ZIP code
 */
export async function getIncentivesByZip(zipCode: string): Promise<StateIncentive[]> {
  const stateCode = getStateFromZip(zipCode);
  if (!stateCode) {
    console.warn(`⚠️ Could not determine state for ZIP: ${zipCode}`);
    return [];
  }
  return getStateIncentives(stateCode);
}
...
export async function calculateIncentives(
  ...
  if (stateCode) {
    ...
    stateIncentives = allIncentives.filter((inc) => {
      ...
      const techMatch = includesSolar
        ? inc.eligible_technologies?.some((t) =>
            ["solar+storage", "battery", "standalone"].includes(t)
          )
    ...
    if (stateIncentives.length > 0) {
      ...
      const rebatePrograms = stateIncentives.filter(
        (i) => i.incentive_per_kwh && i.incentive_per_kwh > 0 && i.program_type === "rebate"
      );
      if (rebatePrograms.length > 0) {
        // Sort by incentive amount descending, take best one
        const bestRebate = rebatePrograms.sort(
          (a, b) => (b.incentive_per_kwh || 0) - (a.incentive_per_kwh || 0)
        )[0];
      ...
      const taxCreditPrograms = stateIncentives.filter(
        (i) => i.incentive_percent && i.incentive_percent > 0 && i.program_type === "tax_credit"
      );
      if (taxCreditPrograms.length > 0) {
        const bestTaxCredit = taxCreditPrograms.sort(
          (a, b) => (b.incentive_percent || 0) - (a.incentive_percent || 0)
        )[0];
  ...
  if (stateTotal > maxStateIncentive) {
    console.warn(
      `⚠️ State incentive capped: $${stateTotal.toLocaleString()} → $${maxStateIncentive.toLocaleString()}`
    );
  ...
  console.log("💰 Incentive Calculation:", {
    ...
    equityProgramsExcluded: allIncentives
      .filter((i) => i.equity_program && !isEquityEligible)
      .map((i) => i.program_name),
  });
...
/**
 * Get incentive summary for a state (for display)
 */
export async function getStateSummary(stateCode: string): Promise<StateIncentiveSummary | null> {
  ...
  const stateName = incentives[0]?.state_name || stateCode;
  const rebates = incentives.filter((i) => i.program_type === "rebate" && i.incentive_per_kwh);
  const taxCredits = incentives.filter(
    (i) => i.program_type === "tax_credit" && i.incentive_percent
  );
  const propertyTax = incentives.some((i) => i.program_type === "property_tax_exemption");
  const equityPrograms = incentives.some((i) => i.equity_program);

  return {
    state_code: stateCode,
    state_name: stateName,
    total_programs: incentives.length,
    best_rebate_per_kwh: Math.max(...rebates.map((r) => r.incentive_per_kwh || 0), 0),
    best_tax_credit_percent: Math.max(...taxCredits.map((t) => t.incentive_percent || 0), 0),
    has_equity_programs: equityPrograms,
    has_property_tax_exemption: propertyTax,
    estimated_incentive: 0, // Calculate based on typical system
  };
}
...
const FALLBACK_INCENTIVES: { [key: string]: Partial<StateIncentive>[] } = {
  CA: [
    { program_name: "SGIP - General Market", incentive_per_kwh: 150, program_type: "rebate" },
    {
      program_name: "SGIP - Equity",
      incentive_per_kwh: 850,
      program_type: "rebate",
      equity_program: true,
    },
  ],
  NY: [{ program_name: "NYSERDA Retail Storage", incentive_per_kwh: 175, program_type: "rebate" }],
  MA: [{ program_name: "ConnectedSolutions", incentive_per_kwh: 225, program_type: "rebate" }],
  NV: [
    {
      program_name: "Property Tax Exemption",
  ...
  MD: [
    {
      program_name: "Energy Storage Tax Credit",
  ...
  AZ: [
    {
      program_name: "Storage Tax Credit",
      incentive_percent: 25,
      max_incentive: 1000,
      program_type: "tax_credit",
    },
  ],
  CO: [{ program_name: "Storage Tax Credit", incentive_percent: 10, program_type: "tax_credit" }],
};

/**
 * Get fallback incentives when database unavailable
 */
export function getFallbackIncentives(stateCode: string): Partial<StateIncentive>[] {
  return FALLBACK_INCENTIVES[stateCode] || [];
}
