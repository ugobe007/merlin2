...
class DailyPricingValidationService {
  ...
  private pricingSources: PricingValidationSource[] = [
    {
      ...
      reliability: "high",
      notes:
        "Cabinet systems with UL9540/9540A certification, grid-forming capability, 3.3h duration",
    },
    {
    ...
    },
    {
      ...
      reliability: "high",
      notes:
        "Utility-scale systems: $75/kWh batteries + $29/kWh inverters = $104/kWh complete system",
    },
    {
      name: "NREL ATB 2024",
  ...
  getLatestValidationResults(): {
    date: string;
    results: ValidationAlert[];
    sources: PricingValidationSource[];
  } | null {
  ...
  addPricingSource(source: PricingValidationSource): void {
    const existingIndex = this.pricingSources.findIndex(
      (s) => s.name === source.name && s.category === source.category
    );
