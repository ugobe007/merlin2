...
export function calculateAutomatedSolarSizing(
  characteristics: BuildingCharacteristics
): SolarSizingResult {
  const useCase = characteristics.useCase || "office";
  const profile =
    SOLAR_SIZING_PROFILES[useCase as keyof typeof SOLAR_SIZING_PROFILES] ||
    SOLAR_SIZING_PROFILES.office;
  ...
  const groundEstimate =
    "groundRatio" in profile ? baseMW * profile.groundRatio * buildingSizeMultiplier : 0;
...
export function calculateSpaceRequirements(
  MW: number,
  installationType: "rooftop" | "ground" | "mixed" = "mixed"
) {
  ...
  return {
    ...
    acres: acres,
    formatted:
      acres >= 1 ? `${acres.toFixed(1)} acres` : `${Math.round(totalSqFt).toLocaleString()} sq ft`,
  };
}
