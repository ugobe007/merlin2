 ...
 */
export const calculateNRELCapex = (inputs: NRELCalculationInputs): {
 ...
 */
export const calculateCapacityFactor = (durationHours: number, cyclesPerDay: number = 1): number => {
 ...
 */
export const calculateOMCosts = (capexPerKW: number): {
 ...
 */
export const calculateCommercialPricing = (inputs: CommercialPricingInputs): {
  costPerKWh: number;
  totalSystemCost: number;
  ...
  const { systemSizeKWh, isContainerized = false, region = 'US', includesInstallation = true, certificationLevel = 'BASIC' } = inputs;
  ...
  const batteryPackCost = totalSystemCost * 0.60; // 60% battery pack
  const bmsCost = totalSystemCost * 0.10; // 10% BMS
  const inverterCost = totalSystemCost * 0.15; // 15% inverter/PCS
  const installationCost = includesInstallation ? totalSystemCost * 0.10 : 0; // 10% installation
 ...
 */
export const calculateBatteryDegradation = (inputs: BatteryDegradationInputs): {
