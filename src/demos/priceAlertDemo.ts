...
async function demo2_saveAlert() {
  ...
  const alert: EnergyPriceAlert = {
    ...
    publishDate: new Date().toISOString(),
    dealSummary:
      "Discovery Energy secured a landmark deal providing 50MW/200MWh battery storage at $118.50/kWh",
...
async function demo3_fetchRecentAlerts() {
  ...
  alerts.forEach((alert, index) => {
    ...
    console.log(
      `   ${alert.vendorCompany || "Unknown vendor"} | ${alert.projectLocation || "Unknown location"}`
    );
    console.log("");
  });
}
...
async function demo4_fetchExcellentDeals() {
  ...
  deals.forEach((deal, index) => {
    ...
    console.log(
      `   Savings: ${Math.abs(deal.priceDifferencePercent || 0).toFixed(1)}% below market`
    );
...
async function demo6_pricingTrends() {
  ...
  console.log(
    `  Declining: ${decliningCount} (${((decliningCount / alerts.length) * 100).toFixed(0)}%)`
  );
  ...
  console.log(
    `  Below Market: ${belowMarketCount} deals (${((belowMarketCount / alerts.length) * 100).toFixed(0)}%)`
  );
