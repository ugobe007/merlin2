...
const POWER_TEST_CASES: TestCase[] = [
  ...
  // HOTELS
  {
    ...
    slug: "hotel",
    inputs: { roomCount: 50 },
    expectedPowerMW: { min: 0.15, max: 0.2 }, // 3.5 kW/room × 50 = 175 kW
    expectedDurationHrs: 4,
  },
  {
    ...
    expectedPowerMW: { min: 0.45, max: 0.6 }, // 3.5 kW/room × 150 = 525 kW
    expectedDurationHrs: 4,
  },
  {
  ...
  // CAR WASH
  {
    ...
    expectedPowerMW: { min: 0.1, max: 0.15 }, // 6 × 20 kW/bay = 120 kW
    expectedDurationHrs: 2,
  },
...
function runPowerTest(testCase: TestCase): TestResult {
  try {
    ...
    const powerInRange =
      result.powerMW >= testCase.expectedPowerMW.min &&
...
export function testPowerCalculations(): { passed: number; failed: number; results: TestResult[] } {
  ...
  for (const testCase of POWER_TEST_CASES) {
    ...
    if (result.passed) {
      ...
      console.log(
        `   Power: ${result.actual.powerMW.toFixed(3)} MW (expected: ${result.expected.powerMW.min}-${result.expected.powerMW.max})`
      );
    } else {
      failed++;
      ...
      console.log(
        `   Actual: ${result.actual.powerMW.toFixed(3)} MW, ${result.actual.durationHrs}hr`
      );
...
export function testDirectCalculators(): void {
  ...
  const tests = [
    ...
    {
      name: "calculateDatacenterPower(undefined, 400, 8)",
      fn: () => calculateDatacenterPower(undefined, 400, 8),
    },
    ...
    {
      name: 'calculateManufacturingPower(100000, "heavy")',
      fn: () => calculateManufacturingPower(100000, "heavy"),
    },
    {
      name: "calculateWarehousePower(250000, false)",
      fn: () => calculateWarehousePower(250000, false),
    },
    {
      name: "calculateWarehousePower(100000, true)",
      fn: () => calculateWarehousePower(100000, true),
    },
    { name: "calculateRetailPower(50000)", fn: () => calculateRetailPower(50000) },
    {
      name: "calculateShoppingCenterPower(500000)",
      fn: () => calculateShoppingCenterPower(500000),
    },
    {
      name: 'calculateAgriculturePower(5000, undefined, "row-crop")',
      fn: () => calculateAgriculturePower(5000, undefined, "row-crop"),
    },
    ...
    {
      name: 'calculateCarWashPower(6, "automatic")',
      fn: () => calculateCarWashPower(6, "automatic"),
    },
  ];
...
export function testCustomInput(slug: string, inputs: Record<string, any>): PowerCalculationResult {
  ...
  console.log("\nResult:");
  console.log(
    `  Power: ${result.powerMW.toFixed(3)} MW (${(result.powerMW * 1000).toFixed(1)} kW)`
  );
