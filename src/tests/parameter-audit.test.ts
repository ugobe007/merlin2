...
describe('Datacenter Tier Standards Audit (Uptime Institute)', () => {
  ...
  test('Tier IV: 60-80% BESS, 4-8 hr duration', () => {
    ...
    expect(tier.bessMultiplier).toBeGreaterThanOrEqual(0.60);
    expect(tier.bessMultiplier).toBeLessThanOrEqual(0.80);
  ...
  test('Print complete parameter table', () => {
    ...
    Object.entries(POWER_DENSITY_STANDARDS).forEach(([key, value]) => {
      const unit = key.includes('Per') ? 
        (key.includes('Room') ? 'kW/room' : key.includes('Bed') ? 'kW/bed' : key.includes('Acre') ? 'kW/acre' : 'MW/M pax') :
        'W/sqft';
    ...
    Object.entries(HOTEL_CLASS_PROFILES).forEach(([key, profile]) => {
      console.log(`  ${profile.name.padEnd(20)} ${profile.kWhPerRoom} kWh/room, ${profile.peakKWPerRoom} kW peak`);
    });
    ...
    Object.entries(DATACENTER_TIER_STANDARDS).forEach(([key, tier]) => {
      console.log(`  ${key.padEnd(8)} ${(tier.bessMultiplier * 100)}% BESS, ${tier.durationHours}hr backup`);
    });
    
    console.log('\n🔌 EV CHARGER POWER LEVELS:');
    console.log('────────────────────────────────────────');
    const evLevels = ['level1', 'level2_7kw', 'level2_11kw', 'level2_19kw', 'level2_22kw', 'dcfc_50kw', 'dcfc_150kw', 'hpc_250kw'];
