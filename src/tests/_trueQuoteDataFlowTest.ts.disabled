...
test('Each step has required fields', () => {
  ...
  const allValid = result.calculationSteps.every((step: any) => 
    step.stepNumber > 0 &&
    step.category &&
    step.name &&
    step.formula &&
...
test('Sources are provided for traceability', () => {
  ...
  const hasNREL = result.sources.some((s: any) => s.shortName.includes('NREL'));
  const hasEIA = result.sources.some((s: any) => s.shortName.includes('EIA'));
  
  return {
    passed: hasSources && hasNREL && hasEIA,
    expected: 'Sources include NREL and EIA',
    actual: result.sources.map((s: any) => s.shortName).join(', '),
