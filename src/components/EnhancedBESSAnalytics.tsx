  ...
  const generateSyntheticLoadProfile = (): LoadProfile[] => {
    ...
    for (let day = 0; day < 30; day++) {
      for (let hour = 0; hour < 24; hour++) {
        ...
        const demand_kW = projectData.storageSizeMW * 1000 * demandMultiplier * (0.8 + 0.4 * Math.random());
        
        data.push({
          timestamp,
          demand_kW: Math.max(10, demand_kW),
          solar_kW: hour >= 6 && hour <= 18 ? projectData.storageSizeMW * 500 * Math.sin((hour - 6) * Math.PI / 12) : 0,
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    ...
                    <p className="text-lg font-bold text-green-600 mt-2">
                      {analysisResults?.optimization.roi_10_year_percent.toFixed(1) || '0'}% 10-year ROI
                    </p>
                  </div>
                </div>
              )}
