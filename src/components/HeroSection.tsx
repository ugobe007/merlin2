...
function RollingNumber({ target, prefix = "", suffix = "", duration = 1200 }: {
...
interface DemoCase {
  ...
  profileQ1: string; profileA1: string;
  profileQ2: string; profileA2: string;
...
const DEMO_CASES: DemoCase[] = [
  {
    zip: "10001", industry: "Manufacturing", industryEmoji: "🏭",
    city: "New York", state: "NY", utility: "Con Edison", rate: 0.22,
    profileQ1: "Monthly energy bill",   profileA1: "$18,400",
    profileQ2: "Facility sq. footage",  profileA2: "42,000 sq ft",
    addOns: ["BESS", "Solar", "Demand Response"],
    solarKW: 480, bessKWh: 1200,
    annualSavings: 621000, paybackYears: 4.2, co2Tons: 310, irr: 24,
  },
  {
    zip: "90210", industry: "Hotel", industryEmoji: "🏨",
    city: "Beverly Hills", state: "CA", utility: "SCE", rate: 0.28,
    profileQ1: "Number of rooms",       profileA1: "214 rooms",
    profileQ2: "Monthly energy bill",   profileA2: "$31,200",
    addOns: ["BESS", "Solar", "EV Charging"],
    solarKW: 620, bessKWh: 1800,
    annualSavings: 412000, paybackYears: 5.1, co2Tons: 198, irr: 19,
  },
  {
    zip: "20001", industry: "Data Center", industryEmoji: "🖥️",
    city: "Washington", state: "DC", utility: "Pepco", rate: 0.14,
    profileQ1: "IT load (kW)",          profileA1: "2,400 kW",
    profileQ2: "Uptime requirement",    profileA2: "99.999% (Tier III)",
    addOns: ["BESS", "Backup Generator", "UPS Integration"],
    solarKW: 0, bessKWh: 4000,
    annualSavings: 534000, paybackYears: 3.8, co2Tons: 440, irr: 26,
  },
  {
    zip: "78701", industry: "Car Wash", industryEmoji: "🚗",
    city: "Austin", state: "TX", utility: "Austin Energy", rate: 0.11,
    profileQ1: "Cars washed per day",   profileA1: "380 cars",
    profileQ2: "Monthly energy bill",   profileA2: "$4,100",
    addOns: ["BESS", "Solar"],
    solarKW: 120, bessKWh: 300,
    annualSavings: 187000, paybackYears: 6.3, co2Tons: 88, irr: 16,
  },
  {
    zip: "60601", industry: "Multifamily", industryEmoji: "🏢",
    city: "Chicago", state: "IL", utility: "ComEd", rate: 0.12,
    profileQ1: "Number of units",       profileA1: "186 units",
    profileQ2: "Common-area bill",      profileA2: "$9,800 / mo",
    addOns: ["BESS", "Solar", "EV Charging"],
    solarKW: 260, bessKWh: 720,
    annualSavings: 298000, paybackYears: 5.7, co2Tons: 162, irr: 18,
  },
  {
    zip: "30301", industry: "EV Charging", industryEmoji: "⚡",
    city: "Atlanta", state: "GA", utility: "Georgia Power", rate: 0.10,
    profileQ1: "Number of stalls",      profileA1: "24 Level 2 / 6 DC Fast",
    profileQ2: "Peak charging demand",  profileA2: "~480 kW",
    addOns: ["BESS", "Solar", "Grid Services"],
    solarKW: 200, bessKWh: 600,
    annualSavings: 156000, paybackYears: 6.8, co2Tons: 74, irr: 15,
  },
];
...
function CalculationCard() {
  ...
  const runCycle = useCallback((caseIdx: number) => {
    clearTimers();
    const c = DEMO_CASES[caseIdx % DEMO_CASES.length];

    // ─── STEP 1 ───────────────────────────────────────────────────────────
    setDemoStep(1); setDemoPhase("idle"); setDemoZip(""); setS3Phase(0); setS4Phase(0); setS5Phase(0);

    let t = 120;
    schedule(() => { setDemoZip(c.zip.slice(0, 3) + "··"); setDemoPhase("locating"); }, t);

    t += S1_LOCATING;
    schedule(() => { setDemoZip(c.zip); setDemoPhase("fetching"); }, t);

    t += S1_FETCHING;
    schedule(() => { setDemoPhase("resolved"); }, t);

    t += S1_RESOLVED + FADE_BETWEEN;

    // ─── STEP 2 ───────────────────────────────────────────────────────────
    schedule(() => { setDemoStep(2); }, t);

    t += S2_DWELL + FADE_BETWEEN;

    // ─── STEP 3 ───────────────────────────────────────────────────────────
    schedule(() => { setDemoStep(3); setS3Phase(1); }, t);          // show q1 label

    t += S3_TYPE_Q;
    schedule(() => { setS3Phase(2); }, t);                           // show a1

    t += S3_SHOW_A;
    schedule(() => { setS3Phase(3); }, t);                           // show q2 label

    t += S3_TYPE_Q;
    schedule(() => { setS3Phase(4); }, t);                           // show a2

    t += S3_SHOW_A + FADE_BETWEEN;

    // ─── STEP 4 ───────────────────────────────────────────────────────────
    schedule(() => { setDemoStep(4); setS4Phase(1); }, t);           // add-on chips appear

    t += S4_CHIPS;
    schedule(() => { setS4Phase(2); }, t);                           // sizing numbers appear

    t += S4_SIZE + FADE_BETWEEN;

    // ─── STEP 5 ───────────────────────────────────────────────────────────
    schedule(() => { setDemoStep(5); setS5Phase(1); }, t);           // counters start

    t += S5_COUNTERS;
    schedule(() => { setS5Phase(2); }, t);                           // payback/CO2/IRR appear
  ...
  useEffect(() => {
    ...
    if (zip.length > 0) {
      ...
      t = setTimeout(() => { setUserActive(false); setDemoCase(0); }, 1200);
    }
  ...
  const handleFocus = useCallback(() => {
    if (!userActive) { setUserActive(true); clearTimers(); setDemoStep(1); setDemoPhase("idle"); setDemoZip(""); }
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.05]">
        <div className="flex items-center gap-2">
          <img src={SHIELD_GOLD} alt="" className="w-5 h-5 object-contain" />
          <span className="text-yellow-400 text-sm font-bold tracking-tight" style={{ fontFamily: "'Outfit', sans-serif" }}>
            TrueQuote
          </span>
          <span className="text-[10px] text-slate-600 font-medium ml-1 uppercase tracking-widest">Engine</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-slate-600"><LiveClock /></span>
        </div>
      </div>
            ...
            <div key={s} className="flex-1 flex flex-col items-center gap-1">
              <div
                className={`h-[3px] w-full rounded-full transition-all duration-500 ${
                  s < activeStep  ? "bg-yellow-500" :
                  s === activeStep ? "bg-yellow-400" :
                                     "bg-white/8"
                }`}
              />
              <span className={`text-[8px] font-mono transition-colors duration-300 ${
                s === activeStep ? "text-yellow-400" : s < activeStep ? "text-yellow-600/50" : "text-slate-700"
              }`}>
                {stepLabels[s-1]}
              </span>
            </div>
      ...
      <div className="px-5 pb-2" style={{ minHeight: 260 }}>
        ...
        {activeStep === 1 && (
          <div key="step1" style={{ animation: "truequote-fadein 0.35s ease-out" }}>
            ...
            <label className="text-[9px] text-slate-600 uppercase tracking-[0.2em] font-semibold block mb-1.5">ZIP Code</label>
            ...
            <div className="h-5">
              {demoPhase === "locating" && (
                <p className="text-[11px] font-mono text-slate-500" style={{ animation: "truequote-fadein 0.3s ease-out" }}>
              ...
              {demoPhase === "resolved" && !userActive && (
                <p key="resolved" className="text-[11px] font-mono text-emerald-400" style={{ animation: "truequote-fadein 0.3s ease-out" }}>
        ...
        {activeStep === 3 && (
          <div key="step3" style={{ animation: "truequote-fadein 0.35s ease-out" }}>
            ...
            <div className="space-y-3">
              {/* Q1 */}
              <div className={`transition-all duration-400 ${s3Phase >= 1 ? "opacity-100" : "opacity-0"}`}>
                ...
                <div className="bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 flex items-center justify-between">
                  <span
                    ...
                    style={{ fontFamily: "'JetBrains Mono', monospace", opacity: s3Phase >= 2 ? 1 : 0, transition: "opacity 0.4s" }}
                  >
                    {dc.profileA1}
                  </span>
                  {s3Phase >= 2 && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />}
                </div>
              </div>
              {/* Q2 */}
              <div className={`transition-all duration-400 ${s3Phase >= 3 ? "opacity-100" : "opacity-0"}`}>
                <label className="text-[9px] text-slate-600 uppercase tracking-widest font-semibold block mb-1">
                  {dc.profileQ2}
                </label>
                <div className="bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 flex items-center justify-between">
                  <span
                    className="text-sm font-bold text-white"
                    style={{ fontFamily: "'JetBrains Mono', monospace", opacity: s3Phase >= 4 ? 1 : 0, transition: "opacity 0.4s" }}
                  >
                    {dc.profileA2}
                  </span>
                  {s3Phase >= 4 && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />}
                </div>
              </div>
              {/* Progress hint */}
              <p className="text-[10px] text-slate-700 font-mono">
                {s3Phase >= 4 ? "✓ Profile complete — continuing..." : "Analyzing facility profile..."}
              </p>
            </div>
          </div>
        )}
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {dc.addOns.map((a, i) => (
                <span
                  ...
                  style={{ fontFamily: "'Outfit', sans-serif", opacity: s4Phase >= 1 ? 1 : 0, transition: `opacity 0.3s ${i * 120}ms` }}
                >
            ...
            >
              {dc.solarKW > 0 && (
                <div className="bg-black/30 border border-white/[0.06] rounded-lg p-3">
                  <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">Solar</div>
                  <div className="text-lg font-extrabold text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                    {dc.solarKW} kW
                  </div>
                </div>
              )}
              {dc.bessKWh > 0 && (
                <div className={`bg-black/30 border border-white/[0.06] rounded-lg p-3 ${dc.solarKW === 0 ? "col-span-2" : ""}`}>
                  <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">BESS Storage</div>
                  <div className="text-lg font-extrabold text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
        ...
        {activeStep === 5 && (
          <div key="step5" style={{ animation: "truequote-fadein 0.35s ease-out" }}>
            ...
            <div className="bg-black/40 border border-emerald-500/20 rounded-xl p-4 mb-3">
              ...
              <div className="text-3xl font-extrabold text-emerald-400 tabular-nums" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                {s5Phase >= 1
                  ? <RollingNumber key={`${dc.city}-savings`} target={dc.annualSavings} prefix="$" duration={1600} />
                  : "$0"}
              </div>
            ...
            >
              <div className="bg-black/30 border border-white/[0.05] rounded-lg p-2.5 text-center">
                <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">Payback</div>
                <div className="text-sm font-extrabold text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  {dc.paybackYears}y
                </div>
              </div>
              <div className="bg-black/30 border border-white/[0.05] rounded-lg p-2.5 text-center">
                <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">CO₂ / yr</div>
                <div className="text-sm font-extrabold text-white" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  {dc.co2Tons}T
                </div>
              </div>
              <div className="bg-black/30 border border-white/[0.05] rounded-lg p-2.5 text-center">
                ...
                <div className="text-sm font-extrabold text-emerald-400" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  {dc.irr}%
                </div>
              </div>
            </div>
      ...
      <div className="px-5 pb-5 pt-3">
        <button
          ...
          style={{ fontFamily: "'Outfit', sans-serif" }}
        >
          {isValid ? "Build my TrueQuote" : activeStep === 5 ? "Enter your ZIP to get started" : "Enter ZIP to begin"}
          ...
          {DEMO_CASES.map((_, i) => (
            <div
              ...
              style={{
                ...
                height:     4,
                background: i === demoCase % DEMO_CASES.length
          ...
          <div className="max-w-2xl">
            ...
            <div className="animate-fade-up-delay-3 mb-4">
              ...
              <div className="flex flex-col sm:flex-row gap-2.5">
                ...
                >
                  <img src={SHIELD_GOLD} alt="TrueQuote" className="w-5 h-5 object-contain flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-yellow-400/90 group-hover:text-yellow-300 transition-colors" style={{ fontFamily: "'Outfit', sans-serif" }}>
                      TrueQuote
                    </div>
                    <div className="text-[11px] text-slate-600">For facility owners &amp; operators</div>
                  </div>
                ...
                >
                  <img src={SHIELD_BLUE} alt="ProQuote" className="w-5 h-5 object-contain flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-blue-400/90 group-hover:text-blue-300 transition-colors" style={{ fontFamily: "'Outfit', sans-serif" }}>
                      ProQuote
                    </div>
