...
const MarketIntelligenceDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "trends" | "configs" | "indicators" | "opportunities" | "adoption" | "pricing"
  >("overview");
    ...
    try {
      ...
      if (fetchError) {
        ...
        if (fetchError.message?.includes("does not exist") || fetchError.code === "42P01") {
          setError(
            "Database table does not exist. Please run the migration: database/migrations/20250103_market_inference_tables.sql"
          );
          return;
        }
      ...
      if (data) {
        ...
        const hasData = marketTrends.length > 0 && bessConfigs.length > 0 && adoption.length > 0;
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to load market intelligence";
  ...
  const handleEdit = async (updatedData: any) => {
    ...
    try {
      ...
      switch (selectedItem.type) {
        case "trend":
          updatedInference.marketTrends = inference.marketTrends.map(
            (t: MarketTrend, idx: number) => (t === selectedItem.data ? updatedData : t)
          );
          break;
        case "config":
          updatedInference.bessConfigurations = inference.bessConfigurations.map(
            (c: BESSConfigurationPattern, idx: number) =>
              c === selectedItem.data ? updatedData : c
          );
          break;
        case "indicator":
          updatedInference.decisionIndicators = inference.decisionIndicators.map(
            (i: CustomerDecisionIndicator, idx: number) =>
              i === selectedItem.data ? updatedData : i
          );
          break;
        case "opportunity":
          updatedInference.emergingOpportunities = inference.emergingOpportunities.map(
            (o: EmergingOpportunity, idx: number) => (o === selectedItem.data ? updatedData : o)
          );
          break;
        case "adoption":
          updatedInference.industryAdoption = inference.industryAdoption.map(
            (a: IndustryAdoptionRate, idx: number) => (a === selectedItem.data ? updatedData : a)
          );
          break;
        case "pricing":
          updatedInference.pricingUpdateRecommendations = (
            inference.pricingUpdateRecommendations || []
          ).map((p: PricingUpdateRecommendation, idx: number) =>
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          pricing_update_recommendations:
            updatedInference.pricingUpdateRecommendations as unknown as Json,
  ...
  const handleDelete = async () => {
    if (!inference || !selectedItem || !confirm("Are you sure you want to delete this item?"))
      return;
    ...
    try {
      ...
      switch (selectedItem.type) {
        case "trend":
          updatedInference.marketTrends = inference.marketTrends.filter(
            (t: MarketTrend) => t !== selectedItem.data
          );
          break;
        case "config":
          updatedInference.bessConfigurations = inference.bessConfigurations.filter(
            (c: BESSConfigurationPattern) => c !== selectedItem.data
          );
          break;
        case "indicator":
          updatedInference.decisionIndicators = inference.decisionIndicators.filter(
            (i: CustomerDecisionIndicator) => i !== selectedItem.data
          );
          break;
        case "opportunity":
          updatedInference.emergingOpportunities = inference.emergingOpportunities.filter(
            (o: EmergingOpportunity) => o !== selectedItem.data
          );
          break;
        case "adoption":
          updatedInference.industryAdoption = inference.industryAdoption.filter(
            (a: IndustryAdoptionRate) => a !== selectedItem.data
          );
          break;
        case "pricing":
          updatedInference.pricingUpdateRecommendations = (
            inference.pricingUpdateRecommendations || []
          ).filter((p: PricingUpdateRecommendation) => p !== selectedItem.data);
          break;
      }
    ...
    <div className="admin-supabase space-y-4">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="admin-title text-xl">Market Intelligence Dashboard</h1>
      ...
      {error && <div className="admin-stroke admin-health-fail admin-stroke-row">{error}</div>}

      {/* Pending Approvals Alert */}
      {pendingApprovals.length > 0 && (
        <div className="admin-stroke admin-health-warn admin-stroke-row">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <AlertCircle className="w-5 h-5 text-amber-600" />
              <span className="font-semibold text-amber-800">
                {pendingApprovals.length} Pricing Update{pendingApprovals.length > 1 ? "s" : ""}{" "}
                Pending Approval
              </span>
            </div>
            <button
      ...
      {!inference && !loading && (
        <div className="admin-stroke text-center py-12">
          <BarChart3 className="w-12 h-12 text-[var(--ink-muted)] mx-auto mb-4" />
          <h3 className="admin-title mb-2">No Analysis Available</h3>
          <p className="admin-subtitle mb-4">Run your first market analysis to see insights</p>
          <button
            type="button"
            onClick={runAnalysis}
        ...
        <>
          ...
          <div className="admin-kpi-grid mb-4">
            <div className="admin-kpi-cell">
              <div className="admin-kpi-label">Market Sentiment</div>
              <div
                className={`admin-kpi-value ${getSentimentColor(inference.overallMarketSentiment).split(" ")[0]}`}
              >
                {inference.overallMarketSentiment.charAt(0).toUpperCase() +
                  inference.overallMarketSentiment.slice(1)}
              </div>
            ...
            <div className="admin-kpi-cell">
              <div className="admin-kpi-label">Data Points</div>
              <div className="admin-kpi-value">{inference.dataPointsAnalyzed.toLocaleString()}</div>
              <div className="admin-subtitle">{inference.sources.length} sources</div>
            </div>

            <div className="admin-kpi-cell">
              <div className="admin-kpi-label">Market Trends</div>
              <div className="admin-kpi-value">{inference.marketTrends.length}</div>
              <div className="admin-subtitle">Active trends</div>
            </div>

            <div className="admin-kpi-cell">
              <div className="admin-kpi-label">Pricing Updates</div>
          ...
          {/* Navigation Tabs */}
          <div className="admin-stroke mb-4">
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 border-b-2 bg-transparent ${
                    activeTab === tab.key
                      ? "border-[var(--magic)] text-[var(--magic)] font-medium"
            ...
            <div className="p-0">
              {activeTab === "overview" && (
                <OverviewTab inference={inference} onItemClick={handleItemClick} />
              )}
              {activeTab === "trends" && (
                <TrendsTab trends={inference.marketTrends} onItemClick={handleItemClick} />
              )}
              {activeTab === "configs" && (
                <ConfigsTab configs={inference.bessConfigurations} onItemClick={handleItemClick} />
              )}
              {activeTab === "indicators" && (
                <IndicatorsTab
                  indicators={inference.decisionIndicators}
                  onItemClick={handleItemClick}
                />
              )}
              {activeTab === "opportunities" && (
                <OpportunitiesTab
                  opportunities={inference.emergingOpportunities}
                  onItemClick={handleItemClick}
                />
              )}
              {activeTab === "adoption" && (
                <AdoptionTab adoption={inference.industryAdoption} onItemClick={handleItemClick} />
              )}
              {activeTab === "pricing" && (
                <PricingTab
...
const OverviewTab: React.FC<{
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-[var(--ink-primary)] mb-4">Market Summary</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-lg p-4 border border-purple-200">
            <div className="admin-subtitle mb-1">Overall Sentiment</div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4 border border-blue-200">
            <div className="admin-subtitle mb-1">Analysis Confidence</div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-[var(--ink-primary)] mb-4">Key Insights</h3>
        <div className="space-y-3">
          <button
            onClick={() =>
              inference.marketTrends[0] && onItemClick("trend", inference.marketTrends[0])
            }
            disabled={!inference.marketTrends[0]}
            className={`admin-stroke admin-stroke-row flex-col !items-start ${
          ...
          >
            ...
            <div className="flex-1 text-left">
              <div className="font-medium text-[var(--ink-primary)] flex items-center gap-2">
                Top Market Trend
                {inference.marketTrends[0] && (
              ...
              <div className="admin-subtitle">
                {inference.marketTrends[0]?.category || "N/A"}:{" "}
                {inference.marketTrends[0]?.direction || "N/A"}
              </div>
            </div>
          </button>
          ...
          <button
            onClick={() =>
              ...
              onItemClick("config", inference.bessConfigurations[0])
            }
          ...
          <button
            onClick={() =>
              ...
              onItemClick("adoption", inference.industryAdoption[0])
            }
          ...
          >
            ...
            <div className="flex-1 text-left">
              ...
              <div className="admin-subtitle">
                {inference.industryAdoption[0]?.industry || "N/A"} (#
                {inference.industryAdoption[0]?.ranking || "N/A"})
              </div>
            </div>
          </button>
        </div>
...
const TrendsTab: React.FC<{
  trends: MarketTrend[];
  onItemClick: (type: "trend", data: MarketTrend) => void;
}> = ({ trends, onItemClick }) => {
  return (
    <div className="space-y-4">
      {trends.map((trend, index) => (
        <button
          ...
          className="w-full text-left admin-stroke admin-stroke-row flex-col !items-start cursor-pointer group"
        >
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-[var(--ink-primary)] capitalize">
                {trend.category}
              </span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="admin-subtitle mb-2">
            Magnitude: {trend.magnitude.toFixed(1)}% | Confidence:{" "}
            {(trend.confidence * 100).toFixed(0)}% | {trend.timeframe} term
          </div>
          <div className="text-xs text-gray-500">{trend.evidence.join(" • ")}</div>
        </button>
      ))}
    </div>
  );
...
const ConfigsTab: React.FC<{
  ...
  onItemClick: (type: "config", data: BESSConfigurationPattern) => void;
}> = ({ configs, onItemClick }) => {
  return (
    <div className="space-y-4">
      {configs.slice(0, 10).map((config, index) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-[var(--ink-primary)]">{config.configuration}</span>
            <div className="flex items-center gap-2">
              <span className="admin-subtitle">{config.frequency} instances</span>
              <ExternalLink className="w-4 h-4 text-gray-400 group-hover:text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </div>
          <div className="admin-subtitle mb-2">
            Avg Price: ${config.avgPrice.toLocaleString()} | Range: $
            {config.priceRange.min.toLocaleString()} - ${config.priceRange.max.toLocaleString()}
          </div>
          <div className="text-xs text-gray-500">
            Industries: {config.industries.join(", ") || "N/A"} | Use Cases:{" "}
            {config.useCases.join(", ") || "N/A"}
          </div>
        </button>
      ))}
    </div>
...
const IndicatorsTab: React.FC<{
  indicators: CustomerDecisionIndicator[];
  onItemClick: (type: "indicator", data: CustomerDecisionIndicator) => void;
}> = ({ indicators, onItemClick }) => {
  return (
    <div className="space-y-4">
      {indicators.slice(0, 10).map((indicator, index) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-[var(--ink-primary)]">{indicator.indicator}</span>
            <div className="flex items-center gap-2">
              <span className="admin-subtitle">{indicator.frequency} mentions</span>
              <ExternalLink className="w-4 h-4 text-gray-400 group-hover:text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          </div>
          <div className="admin-subtitle mb-2">
...
const OpportunitiesTab: React.FC<{
  ...
  onItemClick: (type: "opportunity", data: EmergingOpportunity) => void;
}> = ({ opportunities, onItemClick }) => {
  return (
    <div className="space-y-4">
      {opportunities.map((opp, index) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold text-[var(--ink-primary)]">{opp.opportunity}</span>
            <div className="flex items-center gap-2">
              <span
                className={`px-2 py-1 rounded text-xs font-medium ${
                  opp.marketSize === "large" || opp.marketSize === "very-large"
                    ? "bg-green-100 text-green-700"
                    : opp.marketSize === "medium"
          ...
          <div className="admin-subtitle mb-2">{opp.description}</div>
          <div className="admin-subtitle mb-2">
...
const AdoptionTab: React.FC<{
  adoption: IndustryAdoptionRate[];
  onItemClick: (type: "adoption", data: IndustryAdoptionRate) => void;
}> = ({ adoption, onItemClick }) => {
  return (
    <div className="space-y-4">
      {adoption.map((industry, index) => (
        ...
        >
          ...
          <div className="admin-subtitle mb-2">
            Growth: {industry.growthRate.toFixed(1)}% YoY | Avg System:{" "}
            {industry.avgSystemSize.power.toFixed(0)}kW / {industry.avgSystemSize.energy.toFixed(0)}
            kWh
          </div>
          <div className="text-xs text-gray-500">
...
const PricingTab: React.FC<{
  ...
  return (
    <div className="space-y-6">
      {pendingApprovals.length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-[var(--ink-primary)] mb-4">
            Pending Approvals
          </h3>
          <div className="space-y-4">
            {pendingApprovals.map((approval) => (
              <div key={approval.id} className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                ...
                <div className="admin-subtitle mb-2">
                  Current: ${approval.current_value.toFixed(2)} → Recommended: $
                  {approval.recommended_value.toFixed(2)} ({approval.change_percent > 0 ? "+" : ""}
                  {approval.change_percent.toFixed(1)}%)
                </div>
                <div className="admin-subtitle">{approval.reasoning}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-[var(--ink-primary)] mb-4">Recommendations</h3>
        <div className="space-y-4">
          {recommendations.map((rec, index) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-semibold text-[var(--ink-primary)]">{rec.component}</span>
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2 py-1 rounded text-xs font-medium ${
              ...
              <div className="admin-subtitle mb-2">
                ${rec.currentValue.toFixed(2)} → ${rec.recommendedValue.toFixed(2)} (
                {rec.changePercent > 0 ? "+" : ""}
                {rec.changePercent.toFixed(1)}%)
              </div>
              <div className="admin-subtitle">{rec.reasoning}</div>
            </button>
          ))}
        </div>
      </div>
