...
export async function saveScenario(
  ...
  const peakKw = (quoteResult as any)?.powerProfile?.peakKw || 0;
  const kwhCapacity = (wizardState as any)?.batteryKW
    ? ((wizardState as any).batteryKW * (wizardState as any).durationHours) / 1000
    : 0;
  const totalCost = (quoteResult as any)?.costs?.totalCost || 0;
