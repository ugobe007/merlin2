...
export default function QuestionCard({
  ...
  const normalizedOptions: NormalizedOption[] = Array.isArray(options)
    ? (options as (Record<string, unknown> | string | number)[]).map(
        (o: Record<string, unknown> | string | number, i: number): NormalizedOption => {
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 relative">
                <input
                  type="number"
                  className={`w-full rounded-lg bg-slate-950/60 border px-4 py-3 text-center text-lg font-semibold text-slate-100 border-slate-700/60 focus:border-violet-500 focus:ring-1 focus:ring-violet-500/40 focus:outline-none transition-colors`}
                  value={value === null || value === undefined ? "" : String(value)}
                  onChange={(e) => {
                    const raw = e.target.value;
                    if (raw === "") return setAnswer(q.id, "");
