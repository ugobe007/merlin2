...
import { getStepColors } from '@/components/wizard/constants/stepColors';
import { MerlinHat } from '@/components/wizard/MerlinHat';
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-3xl p-8 border-2 border-[#68BFFA] shadow-xl mb-6">
          ...
          {!showInternational ? (
            ...
            {/* Location Type (Urban/Suburban/Rural) - Enhanced Panel */}
            {wizardState.state && (
              <div className="mt-6">
                <div className="bg-gradient-to-br from-[#e0c3fc] via-[#8ec5fc] to-[#a9c9ff] rounded-2xl shadow-xl p-6 border-2 border-[#6700b6]/20">
                  <label className="block text-base font-bold text-[#6700b6] mb-4 tracking-wide drop-shadow-sm">
                    <span className="inline-flex items-center gap-2"><MapPin className="w-5 h-5 text-[#6700b6]" /> Location Type</span>
                  </label>
                    ...
                    {LOCATION_TYPES.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 transition-all shadow-md ${
                            isSelected
                              ? 'bg-gradient-to-br from-[#6700b6] to-[#060F76] text-white border-[#6700b6] scale-105 shadow-lg'
                              : 'bg-white text-[#6700b6] border-transparent hover:bg-[#f3e8ff] hover:border-[#6700b6]/40 hover:scale-102'
                          }`}
                        >
                          <Icon className="w-7 h-7 mb-1 drop-shadow-md" />
                          <span className="font-semibold text-base">{type.label}</span>
                          <span className="text-xs text-[#6700b6]/70 mt-1">{type.description}</span>
                        </button>
                      );
                    })}
                  </div>
            ...
            {/* Solar Input Field - Enhanced Panel */}
            {wizardState.state && getStateData(wizardState.state).solar > 3.5 && (
              <div className="mt-6">
                <div className="bg-gradient-to-br from-[#fffbe6] via-[#ffe5b2] to-[#ffd59e] rounded-2xl shadow-xl p-6 border-2 border-[#ffa600]/30">
                  <label className="block text-base font-bold text-[#b26a00] mb-4 tracking-wide drop-shadow-sm">
                    <span className="inline-flex items-center gap-2"><Sun className="w-5 h-5 text-[#ffa600]" /> Solar System Size (kW)</span>
                    <span className="ml-2 text-xs text-amber-600 font-normal">(based on your location's solar potential)</span>
                  </label>
                  <div className="flex items-center gap-6">
                    <input
                      ...
                      onChange={e => setWizardState(prev => ({ ...prev, solarKW: Number(e.target.value) }))}
                      className="flex-1 h-3 bg-gray-200 rounded-full appearance-none cursor-pointer accent-[#ffa600] shadow-inner"
                      style={{
                        background: `linear-gradient(to right, #ffa600 0%, #ffa600 ${((wizardState.solarKW || 0) / 1000) * 100}%, #e5e7eb ${((wizardState.solarKW || 0) / 1000) * 100}%, #e5e7eb 100%)`
                      }}
                    />
                    <div className="w-28 text-center flex flex-col items-center">
                      <span className="text-3xl font-black text-[#ffa600] drop-shadow-md">{wizardState.solarKW || 0}</span>
                  ...
                  <div className="mt-2 text-xs text-emerald-700 font-semibold">
                    {getStateData(wizardState.state).solar >= 5.0
                      ? 'Excellent solar resource! Consider maximizing your system.'
                      : getStateData(wizardState.state).solar >= 4.0
                      ? 'Good solar potential for your area.'
            ...
            {/* Multi-Location Toggle - Enhanced Panel */}
            {wizardState.state && (
              <div className="mt-4">
                <div className="bg-gradient-to-br from-[#fffbe6] via-[#ffe5b2] to-[#ffd59e] rounded-2xl shadow-xl p-6 border-2 border-[#ffa600]/30 flex flex-col gap-2">
                  <button
                    onClick={() => setIsMultiLocation(!isMultiLocation)}
                    className={`w-full flex items-center justify-between px-5 py-4 rounded-xl transition-all border-2 shadow-md ${
                      isMultiLocation
                        ? 'bg-[#ffa600]/20 border-[#ffa600] shadow-lg scale-105'
                        : 'bg-white border-transparent hover:bg-[#fff3cd] hover:border-[#ffa600]/40 hover:scale-102'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <Layers className={`w-6 h-6 ${isMultiLocation ? 'text-[#ffa600]' : 'text-gray-500'} drop-shadow-md`} />
                      <span className={`font-semibold text-lg ${isMultiLocation ? 'text-[#9d6200]' : 'text-gray-700'}`}>Multiple Locations (Chain)</span>
                    </div>
                  ...
                  {isMultiLocation && (
                    <p className="mt-3 text-sm text-gray-500 px-2 flex items-center gap-2">
