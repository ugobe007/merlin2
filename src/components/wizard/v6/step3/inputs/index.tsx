      ...
      {options.map((option) => {
        ...
        return (
          ...
          >
            {/* Selection checkmark */}
            {isSelected && (
              <div className={cn('absolute top-2 right-2 w-5 h-5 rounded-full flex items-center justify-center', scheme.iconBgSelected)}>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-center">
        <div className={cn('px-4 py-2 rounded-lg font-semibold text-lg', scheme.iconBg, scheme.textSelected)}>
          {displayValue}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 relative h-3 bg-slate-200 rounded-full overflow-hidden">
          {/* Filled portion */}
          <div
            className={cn('absolute left-0 top-0 h-full bg-gradient-to-r rounded-full transition-all duration-150', scheme.primaryGradient)}
          ...
          {/* Thumb indicator */}
          <div
            className={cn('absolute top-1/2 -translate-y-1/2 w-5 h-5 bg-white border-2 rounded-full shadow-md transition-all duration-150 pointer-events-none', scheme.borderSelected)}
      ...
      {allowEstimate && onEstimateToggle && (
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            className={cn('w-4 h-4 rounded border-slate-300 focus:ring-2', `text-${scheme.primary.replace('500', '600')}`, scheme.ring)}
          />
...
export const SectionHeader: React.FC<SectionHeaderProps> = ({
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-slate-200 mb-4">
      <div className="flex items-center gap-3">
        {icon && (
          <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center bg-gradient-to-br', scheme.primaryGradient, 'text-white')}>
...
export const SmartQuestion: React.FC<SmartQuestionProps> = ({
  ...
  const renderInput = () => {
    switch (question_type) {
      ...
      case 'select':
        ...
        return (
          <select
            ...
            className={cn('w-full px-4 py-3 rounded-xl border-2 border-slate-200 bg-white text-slate-700 focus:outline-none focus:ring-2', scheme.focusBorder, scheme.ring)}
          >
      ...
      case 'number':
        const numConfig = config || { min: 0, max: 1000, step: 1 };
        const range = (numConfig.max || 1000) - (numConfig.min || 0);
        
        if (range > 20) {
          return (
            <SliderWithButtons
              value={value ?? numConfig.default ?? numConfig.min ?? 0}
              onChange={(v) => onChange(field_name, v)}
              min={numConfig.min ?? 0}
  ...
  return (
    <div className="space-y-2">
      <label className="block">
        ...
        {help_text && (
          <span className="block text-xs text-slate-500 mt-0.5">{help_text}</span>
        )}
      </label>
      {renderInput()}
    </div>
