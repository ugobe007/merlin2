...
import { Upload, FileText, DollarSign, CheckCircle, Clock, TrendingUp, Building2, AlertCircle, Loader2, Calculator, Sparkles, ShieldCheck, Home, ArrowLeft, BarChart3, Zap, ArrowUpRight } from "lucide-react";
...
function UsageMeter({ label, used, limit, unlimited, color = 'emerald' }: {
  label: string;
  used: number;
  limit: number;
  unlimited: boolean;
  ...
  const barColor = isNearLimit
    ? 'bg-amber-500'
    : color === 'cyan' ? 'bg-cyan-500' : color === 'blue' ? 'bg-blue-500' : 'bg-emerald-500';

  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs font-medium text-white/50">{label}</span>
        <span className={`text-xs font-semibold ${isNearLimit ? 'text-amber-400' : 'text-white/70'}`}>
...
const CATEGORY_LABELS: Record<string, string> = {
  bess: 'BESS', solar: 'Solar', wind: 'Wind', inverters: 'Inverters',
  transformers: 'Transformers', generators: 'Generators', ev_chargers: 'EV Chargers',
  bos: 'Balance of System', nuclear_smr: 'Nuclear / SMR',
};

const TIER_DISPLAY: Record<string, { label: string; color: string }> = {
  vendor_starter: { label: 'Starter', color: 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' },
  vendor_pro: { label: 'Pro', color: 'text-cyan-400 border-cyan-500/30 bg-cyan-500/10' },
  vendor_enterprise: { label: 'Enterprise', color: 'text-blue-400 border-blue-500/30 bg-blue-500/10' },
};

function VendorUsagePanel() {
    ...
    <div className="rounded-xl border border-white/[0.06] bg-[rgba(255,255,255,0.03)] overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold border ${tierInfo.color}`}>
            {tierInfo.label}
          </span>
        </div>
      ...
      <div className="px-6 py-4 border-t border-white/[0.06] bg-white/[0.01]">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-white/70">{summary.impressions.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-2">
            <TrendingUp className="w-3.5 h-3.5 text-emerald-400" />
            <span className="text-xs text-white/40">Click-throughs</span>
            <span className="text-xs font-semibold text-white/70">{summary.clickThroughs.toLocaleString()}</span>
          </div>
          <div className="flex items-center gap-2">
        ...
        {summary.categories.enabled.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {summary.categories.enabled.map((cat) => (
              <span key={cat} className="px-2 py-0.5 rounded text-[10px] font-medium text-cyan-400 bg-cyan-500/10 border border-cyan-500/20">
