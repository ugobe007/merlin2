...
const SmartWizardV2: React.FC<SmartWizardProps> = ({ show, onClose, onFinish, startInAdvancedMode = false, onOpenAdvancedQuoteBuilder, skipIntro = false }) => {
  // ✅ PERFORMANCE: Only log in development mode to reduce console overhead
  // Removed excessive logging - was causing 180+ console messages
  ...
  // Clear useCaseData ONLY when template changes to a DIFFERENT template
  useEffect(() => {
    if (selectedTemplate && previousTemplate && selectedTemplate !== previousTemplate) {
      console.log(`🔄 Template changed from ${previousTemplate} to ${selectedTemplate}, clearing useCaseData`);
      setUseCaseData({});
    }
    if (selectedTemplate) {
  ...
  // ✅ Initialize with small default to prevent display glitch, will be updated by calculateConfig
  const [storageSizeMW, setStorageSizeMW] = useState<number>(0.1); // Small non-zero value
  const [durationHours, setDurationHours] = useState(4);
  
  // ✅ Add calculating state to show loading indicator
  const [isCalculatingBaseline, setIsCalculatingBaseline] = useState(false);
  
  // Baseline calculation result (for passing to Step3)
    ...
    const calculateConfig = async () => {
      if (selectedTemplate && Object.keys(useCaseData).length > 0 && !isQuickstart) {
        ...
        switch (selectedTemplate) {
          case 'hotel':
            scale = parseInt(useCaseData.numberOfRooms || useCaseData.numRooms) || 100; // Number of rooms (use actual count)
            break;
          case 'car-wash':
        ...
        // Store baseline result for Step3 to use
        setBaselineResult({
          ...
          solarMW: baseline.solarMW // ✅ ADD: Pass calculated solar to Step 3 for preconfiguration
        });
        
        // 🔧 FIX: Initialize solar slider with calculated value when user expressed interest
        if (baseline.solarMW && baseline.solarMW > 0 && useCaseData?.hasSolarInterest) {
          setSolarMW(baseline.solarMW);
          setIncludeRenewables(true); // Auto-enable renewables section
          if (import.meta.env.DEV) {
            console.log('☀️ [Solar Preconfigured]:', {
              calculatedSolarMW: baseline.solarMW,
              userSelection: useCaseData.solarAvailableSpace,
              autoEnabled: true
            });
          }
        }
        ...
        if (baseline.generationRequired && baseline.generationRecommendedMW && baseline.generationRecommendedMW > 0) {
          ...
          // DO NOT auto-enable renewables - let user decide in Step 3/4
          // The warning will appear in Step 3 to guide the user
          ...
          // User will see it as a suggestion in Step 3
        }
    ...
    if (step === 2 && Object.keys(useCaseData).length > 0 && selectedTemplate) {
      ...
      switch (selectedTemplate) {
        ...
        case 'hotel':
          ...
          // Hotel solar: Use baselineService calculation based on user's solarAvailableSpace selection
          // Typical hotel: 2-3 kW per room for rooftop, more if ground-mount available
          let hotelSolarMW = numRooms * 0.0025; // Conservative 2.5kW per room average
        ...
        case 'datacenter':
          ...
          // Data centers: Scale solar with facility size (20-30% of capacity typical)
          let datacenterSolarMW = capacity * 0.3; // 30% of IT capacity for realistic solar offset
  ...
  */
  ...
  const calculateCosts = useCallback(async () => {
    try {
      ...
      const financialMetrics = await calculateFinancialMetrics({
        ...
        includeBackupValue: includeBackupValue,
        includeNPV: true
      });
      ...
      const calculatedCosts = {
        ...
        totalProjectCost: financialMetrics.totalProjectCost,
        taxCredit: financialMetrics.taxCredit,
      ...
      console.error('❌ [calculateCosts] Error in cost calculation:', error);
      // Return fallback costs
      const fallbackCosts = {
        equipmentCost: storageSizeMW * durationHours * 300000,
        installationCost: storageSizeMW * durationHours * 100000,
        shippingCost: 0,
        tariffCost: 0,
        totalProjectCost: storageSizeMW * durationHours * 400000,
        taxCredit: storageSizeMW * durationHours * 400000 * 0.3,
        netCost: storageSizeMW * durationHours * 400000 * 0.7,
        annualSavings: storageSizeMW * 50000,
        paybackYears: (storageSizeMW * durationHours * 400000 * 0.7) / (storageSizeMW * 50000)
      };
  ...
  useEffect(() => {
    const updateCosts = async () => {
      try {
        ...
        console.error('❌ [SmartWizardV2] Error calculating costs:', error);
        // Set fallback costs to prevent showing $0
        setCosts({
          equipmentCost: storageSizeMW * durationHours * 300000, // Fallback: $300/kWh
          installationCost: storageSizeMW * durationHours * 100000, // Fallback: $100/kWh
          shippingCost: 0,
          tariffCost: 0,
          totalProjectCost: storageSizeMW * durationHours * 400000,
          taxCredit: storageSizeMW * durationHours * 400000 * 0.3,
          netCost: storageSizeMW * durationHours * 400000 * 0.7,
          annualSavings: storageSizeMW * 50000, // Fallback: $50k per MW
          paybackYears: (storageSizeMW * durationHours * 400000 * 0.7) / (storageSizeMW * 50000)
        });
      }
    };
  ...
  const getStepTitle = () => {
    const titles = [
      ...
      'Add Power Generation',           // Step 3 - Clearer: tells user to ADD power, not just "options"
    ...
    switch (step) {
      ...
      case 1:
        // 🔍 DEBUG: Log what template is being passed to Step2
        if (import.meta.env.DEV && selectedTemplate === 'office') {
          console.log('🏢 [SmartWizardV2] Rendering Step2 with office template:', {
            selectedTemplate,
            useCaseDataKeys: Object.keys(useCaseData),
            useCaseDataValues: useCaseData
          });
        }
      ...
      case 3:
        return (
          <Step3_AddRenewables
            ...
            setGeneratorsExplicitlyAdded={setGeneratorsExplicitlyAdded}
            storageSizeMW={storageSizeMW}
            useCase={selectedTemplate}
            buildingSize={useCaseData.buildingSize || useCaseData.facilitySize}
            facilitySize={useCaseData.facilitySize}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full my-8 max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {/* Fixed Power Status Bar - Show from Step 2 onwards (Simple Configuration through Quote Summary) */}
        {step >= 2 && step <= 5 && (
          <div className="sticky top-0 z-10 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 text-white px-6 py-3 border-b-2 border-slate-600 shadow-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <span className="text-2xl">⚡</span>
                <div>
                  <p className="text-xs opacity-70 uppercase tracking-wide">Power Configuration</p>
                  <p className="font-bold text-lg">{storageSizeMW.toFixed(1)} MW System</p>
                </div>
              </div>
              
              {baselineResult?.generationRequired && (
              ...
              {baselineResult && !baselineResult.generationRequired && baselineResult.gridCapacity !== undefined && (
                <div className="flex items-center gap-2 bg-green-500/20 border border-green-400/30 rounded-lg px-4 py-2">
                  <span className="text-xl">✅</span>
                  <div className="text-right">
                    <p className="text-xs opacity-80">Grid Capacity</p>
                    <p className="font-bold">Adequate</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2 text-xs">
                <div className="bg-white/10 rounded px-3 py-1 text-center">
                  <p className="opacity-70">Peak</p>
                  <p className="font-bold">{baselineResult?.peakDemandMW?.toFixed(1) || storageSizeMW.toFixed(1)} MW</p>
                </div>
                <div className="bg-white/10 rounded px-3 py-1 text-center">
                  <p className="opacity-70">Grid</p>
                  <p className="font-bold">{baselineResult?.gridCapacity?.toFixed(1) || 0} MW</p>
                </div>
                <div className="bg-white/10 rounded px-3 py-1 text-center">
                  <p className="opacity-70">Battery</p>
                  <p className="font-bold">{storageSizeMW.toFixed(1)} MW</p>
                </div>
                <div className="bg-white/10 rounded px-3 py-1 text-center">
                  <p className="opacity-70">
                    {generatorMW > 0 && solarMW === 0 && windMW === 0 ? 'Generators' :
                     solarMW > 0 && generatorMW === 0 && windMW === 0 ? 'Solar' :
                     windMW > 0 && solarMW === 0 && generatorMW === 0 ? 'Wind' :
                     'Generation'}
                  </p>
                  <p className="font-bold">
                    {(solarMW + windMW + generatorMW) > 0 
                      ? (solarMW + windMW + generatorMW).toFixed(1)
                      : baselineResult?.generationRequired 
                        ? (baselineResult.generationRecommendedMW?.toFixed(1) || '0')
                        : '0'
                    } MW
                  </p>
                  {/* Show indicator if displaying recommended vs actual */}
                  {(solarMW + windMW + generatorMW) === 0 && baselineResult?.generationRequired && (
                    <p className="text-[10px] text-yellow-300 opacity-90">(recommended)</p>
                  )}
                </div>
              </div>
            </div>
