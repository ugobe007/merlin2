  ...
  useEffect(() => {
    const calculateConfig = async () => {
      if (selectedTemplate && Object.keys(useCaseData).length > 0 && !isQuickstart) {
        ...
        // ✅ CRITICAL FIX: Set solar from baseline calculation!
        if (baseline.solarMW > 0) {
          setSolarMW(baseline.solarMW);
          if (import.meta.env.DEV) {
            console.log('☀️ [SmartWizard] Setting solar from baseline:', baseline.solarMW);
          }
        }
  ...
  */
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 1: {
        // For Step 1 (Use Case Questions), check if core required fields are answered
        const hasSquareFootage = useCaseData.squareFootage !== undefined && useCaseData.squareFootage > 0;
        const hasFacilitySize = useCaseData.facilitySize !== undefined && useCaseData.facilitySize !== '';
        const hasOperatingHours = useCaseData.operatingHours !== undefined && useCaseData.operatingHours > 0;
        const hasGridConnection = useCaseData.gridConnection !== undefined && useCaseData.gridConnection !== '';
        
        // Office building needs: squareFootage, facilitySize, operatingHours, gridConnection (all required)
        if (selectedTemplate === 'office-building' || selectedTemplate === 'office') {
          const allRequired = hasSquareFootage && hasFacilitySize && hasOperatingHours && hasGridConnection;
          if (import.meta.env.DEV) {
            console.log('🔍 [canProceed] Office validation:', {
              hasSquareFootage,
              hasFacilitySize,
              hasOperatingHours,
              hasGridConnection,
        ...
        // For other industries, allow if ANY questions are answered
    ...
    return (
      <QuoteCompletePage
        quoteData={{
          ...
          equipmentBreakdown // ✅ Pass actual equipment costs from calculation
        }}
        ...
        {step >= 2 && step <= 5 && baselineResult && (
          <div className="sticky top-0 z-10 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 text-white px-6 py-3 border-b-2 border-slate-600 shadow-lg">
            <div className="flex items-center justify-between">
              ...
              {!baselineResult.generationRequired && baselineResult.gridCapacity !== undefined && (
              ...
              <div className="flex gap-2 text-xs">
                ...
                <div className="bg-white/10 rounded px-3 py-1 text-center">
                  ...
                  <p className="font-bold">{(solarMW + windMW + generatorMW).toFixed(1)} MW</p>
                </div>
              </div>
            </div>
          </div>
