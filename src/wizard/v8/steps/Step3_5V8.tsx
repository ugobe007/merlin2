...
type FuelType = "diesel" | "natural-gas" | "dual-fuel" | "linear";
...
function BackupGeneratorCard({
  ...
  const fuelOptions: {
    ...
    },
    {
      ...
      icon: "🔄",
      label: "Linear",
      desc: "Continuous baseload · trickle-charges BESS · 72h endurance",
    },
  ];
      ...
      >
        <div style={{ display: "flex", gap: 7, alignItems: "flex-start" }}>
          ...
          >
            <span style={{ fontSize: 18 }}>🧙 </span>
            <strong
              style={{ color: fuelType === "linear" ? "#22d3ee" : "#fb923c", fontWeight: 700 }}
            >
              Merlin suggests {safeRec.toLocaleString()} kW
              {fuelType === "linear" ? " for continuous baseload output" : " for power generation"}
            </strong>{" "}
            {fuelType === "linear"
              ? `Sized at 1.2× base load to trickle-charge the BESS during normal operations and sustain 72h+ autonomy during grid outages.`
              : peakLoadKW > 0
                ? `to cover your ${peakLoadKW.toLocaleString()} kW peak load with 1.25× reserve margin.`
                : "based on your facility requirements."}
            {isFullBackup && fuelType !== "linear" && (
...
export default function Step3_5V8({ state, actions }: Props) {
  ...
  const [fuelType, setFuelType] = useState<FuelType>(() => {
    ...
    if ((state.linearGeneratorKW ?? 0) > 0 && !(state.generatorKW > 0)) return "linear";
    return (state.generatorFuelType as FuelType) ?? "natural-gas";
  });
  ...
  const handleGeneratorConfig = (kw: number) => {
    if (fuelType === "linear") {
      actions.setAddonConfig({ generatorKW: 0, linearGeneratorKW: kw });
    } else {
      actions.setAddonConfig({ generatorKW: kw, linearGeneratorKW: 0 });
    }
  };
  ...
  const handleFuelType = (fuel: FuelType) => {
    setFuelType(fuel);
    // Migrate kW to the correct state field when switching between rotary and linear
    const currentKW =
      fuelType === "linear"
        ? (state.linearGeneratorKW ?? 0) > 0
    ...
    if (fuel === "linear") {
      actions.setAddonConfig({
        ...
        generatorKW: 0,
        linearGeneratorKW: currentKW,
      });
    } else {
      actions.setAddonConfig({
        generatorFuelType: fuel,
        generatorKW: currentKW,
        linearGeneratorKW: 0,
      });
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      {/* ── Guided header — frames the step & points to the CTA ── */}
      <div
        style={{
          borderRadius: 16,
          padding: "20px 22px",
          background:
            "linear-gradient(135deg, rgba(155,109,255,0.10), rgba(79,138,255,0.07) 55%, rgba(56,189,248,0.06))",
          border: "1px solid rgba(99,120,255,0.26)",
        }}
      >
        <div
          style={{
        ...
        >
          Build your energy stack{city && city !== "Your Facility" ? `, ${city}` : ""}
        </h1>
        <p
          style={{
            margin: "9px 0 0",
        ...
        >
          Battery storage is your foundation — it's already included. Add any of the optional
          boosters below to increase your ROI, then build your system.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
          {[
            { icon: "⭐", label: "Solar", tone: "#fbbf24" },
            { icon: "⚡", label: "EV Charging", tone: "#38bdf8" },
            { icon: "🔥", label: "Backup Power", tone: "#fb923c" },
          ].map((b) => (
            <span
              key={b.label}
              style={{
      ...
      {/* ── Section label — the decisions ── */}
      <div
        style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 2, flexWrap: "wrap" }}
      >
        <span
          style={{
            fontSize: 11,
        ...
        <span style={{ fontSize: 12, color: "rgba(148,163,184,0.62)" }}>
          Each one is optional — adjust the size and watch your ROI update below.
        </span>
      </div>
      ...
      {wantsGenerator ? (
        <>
          ...
          <BackupGeneratorCard
            ...
            minKW={fuelType === "linear" ? linearGenMinKW : genMinKW}
            maxKW={fuelType === "linear" ? linearGenMaxKW : genMaxKW}
            recKW={fuelType === "linear" ? linearGenRecKW : genRecKW}
            initialKW={
              fuelType === "linear"
                ? (state.linearGeneratorKW ?? 0) > 0
                  ? (state.linearGeneratorKW ?? 0)
      ...
      {/* ── Primary CTA — the single, unmistakable decision point ── */}
      <div style={{ marginTop: 4 }}>
        <button
          ...
          style={{
            ...
            border: "none",
            cursor: "pointer",
            background: "linear-gradient(90deg, #9b6dff 0%, #4f8aff 52%, #38bdf8 100%)",
            color: "#0a0e27",
            fontSize: 17,
            fontWeight: 900,
            letterSpacing: "0.01em",
            ...
            justifyContent: "center",
            gap: 10,
            boxShadow: "0 12px 32px rgba(79,138,255,0.35)",
            transition: "transform 0.12s ease, box-shadow 0.12s ease",
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.transform = "translateY(-1px)";
            e.currentTarget.style.boxShadow = "0 16px 40px rgba(79,138,255,0.48)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.transform = "translateY(0)";
            e.currentTarget.style.boxShadow = "0 12px 32px rgba(79,138,255,0.35)";
          }}
        >
          ⚡ Build My Energy Stack →
        </button>
        <div
          style={{
            textAlign: "center",
        ...
        >
          MagicFit sizes your battery + add-ons into{" "}
          <strong style={{ color: "rgba(226,232,240,0.92)" }}>3 quote options</strong> — no
          commitment.
        </div>
      </div>
    </div>
