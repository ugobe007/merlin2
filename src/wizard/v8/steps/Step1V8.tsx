...
const GOOGLE_MAPS_API_KEY = import.meta.env.VITE_GOOGLE_MAPS_API_KEY || 'AIzaSyB9VeakhIGZQgCKmTiZ3ml0RvnvlT0dNrY';
...
export function Step1V8({ state, actions }: Step1Props) {
  ...
  const normalizedZip = country === "US"
    ? locationRaw.replace(/\D/g, "").slice(0, 5)
    : locationRaw.trim();

  const isValidZip = country === "US"
  ...
  const showIntelStrip = isValidZip && (
    intel !== null ||
  ...
  const handleBusinessSearch = () => {
    ...
    if (typeof window !== 'undefined' && 
      ...
      try {
        // Search Google Places for business details
        const service = new window.google.maps.places.PlacesService(document.createElement('div'));
        const request = {
          query: `${businessName.trim()} near ${normalizedZip}`,
          fields: ['name', 'formatted_address', 'place_id', 'photos', 'geometry'],
        };
        ...
        service.textSearch(request, (results, status) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK && results && results[0]) {
            ...
            // Set business with Google Places data
      ...
      // Google Maps API not loaded - use manual input
      actions.setBusiness(businessName.trim());
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 11, color: T.textMuted, margin: 0, letterSpacing: "0.04em", fontWeight: 500 }}>
      ...
      {error && (
        <div style={{
          ...
          <button
            ...
            style={{ color: "#fca5a5", textDecoration: "underline", background: "none", border: "none", cursor: "pointer", fontSize: 12 }}
          >
            dismiss
          </button>
        </div>
      ...
      {locationConfirmed && !state.business && (
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
            {([
              { value: 'reliable', label: '✅ Reliable', desc: 'Rare outages', color: 'rgba(34,197,94,0.20)', activeColor: 'rgba(34,197,94,0.50)' },
              { value: 'occasional-outages', label: '⚠️ Occasional', desc: 'Few times/year', color: 'rgba(251,191,36,0.20)', activeColor: 'rgba(251,191,36,0.50)' },
              { value: 'frequent-outages', label: '🔴 Frequent', desc: 'Monthly issues', color: 'rgba(249,115,22,0.20)', activeColor: 'rgba(249,115,22,0.50)' },
              { value: 'unreliable', label: '💥 Unreliable', desc: 'Weekly outages', color: 'rgba(239,68,68,0.20)', activeColor: 'rgba(239,68,68,0.50)' },
            ] as const).map((option) => (
              <button
                key={option.value}
                type="button"
                ...
                style={{
                  padding: 12,
                  borderRadius: 10,
                  border: state.gridReliability === option.value ? `2px solid ${option.activeColor}` : "1px solid rgba(255,255,255,0.08)",
                  background: state.gridReliability === option.value ? option.color : "rgba(255,255,255,0.02)",
                  color: T.textPrimary,
                  fontSize: 12,
                  fontWeight: 600,
                  cursor: "pointer",
              ...
              >
                ...
                <div style={{ fontSize: 10, color: T.textMuted, fontWeight: 400 }}>{option.desc}</div>
              </button>
            ))}
          </div>

          {(state.gridReliability === 'frequent-outages' || state.gridReliability === 'unreliable') && (
            <div style={{
              marginTop: 12,
              padding: 10,
              borderRadius: 8,
              ...
              <span>🔋</span>
              <span style={{ flex: 1 }}>
                <strong>Backup generator recommended.</strong> We'll include it in your quote automatically.
              </span>
            </div>
          )}
        </div>
      ...
      {locationConfirmed && !state.business && (
        <div style={{
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: T.textSub, lineHeight: 1.5 }}>
              Let Merlin know if you want solar, EV charging, or backup power — we'll optimize your BESS sizing accordingly.
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
            ...
            <button
              ...
              style={{
                padding: 14,
                borderRadius: 10,
                border: state.wantsSolar ? "2px solid rgba(251,191,36,0.50)" : "1px solid rgba(255,255,255,0.08)",
            ...
            >
              <div style={{
                ...
                height: 20,
                borderRadius: 6,
                border: state.wantsSolar ? "2px solid #fbbf24" : "2px solid rgba(255,255,255,0.15)",
            ...
            <button
              ...
              style={{
                padding: 14,
                borderRadius: 10,
                border: state.wantsEVCharging ? "2px solid rgba(59,130,246,0.50)" : "1px solid rgba(255,255,255,0.08)",
                background: state.wantsEVCharging ? "rgba(59,130,246,0.08)" : "rgba(255,255,255,0.02)",
                color: T.textPrimary,
                fontSize: 13,
                fontWeight: 600,
                cursor: "pointer",
            ...
            >
              <div style={{
                ...
                height: 20,
                borderRadius: 6,
                border: state.wantsEVCharging ? "2px solid #3b82f6" : "2px solid rgba(255,255,255,0.15)",
            ...
            <button
              ...
              style={{
                padding: 14,
                borderRadius: 10,
                border: state.wantsGenerator ? "2px solid rgba(239,68,68,0.50)" : "1px solid rgba(255,255,255,0.08)",
                background: state.wantsGenerator ? "rgba(239,68,68,0.08)" : "rgba(255,255,255,0.02)",
                color: T.textPrimary,
                fontSize: 13,
                fontWeight: 600,
                cursor: "pointer",
            ...
            >
              <div style={{
                ...
                height: 20,
                borderRadius: 6,
                border: state.wantsGenerator ? "2px solid #ef4444" : "2px solid rgba(255,255,255,0.15)",
              ...
              <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {(state.gridReliability === 'frequent-outages' || state.gridReliability === 'unreliable') && state.wantsGenerator && (
                  <span style={{
                    fontSize: 9,
                    padding: "2px 6px",
                    borderRadius: 4,
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 10, fontWeight: 500, color: T.textMuted, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                Skip industry selection
              </div>
            </div>
          </div>

          <p style={{ fontSize: 12, color: "rgba(255,255,255,0.40)", margin: "0 0 14px 44px", lineHeight: 1.6 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input
              type="text"
              value={businessName}
              onChange={(e) => { setBusinessName(e.target.value); setBusinessError(null); }}
              onKeyDown={(e) => { if (e.key === "Enter") handleBusinessSearch(); }}
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{
              ...
              Street Address{" "}
              <span style={{ opacity: 0.6, fontWeight: 400, textTransform: "none" }}>(optional — improves map accuracy)</span>
            </label>
            <input
              type="text"
              value={addressValue}
          ...
          <button
            ...
            style={{
              ...
              padding: "11px 18px",
              borderRadius: 8,
              border: !businessName.trim() || isFetching
                ? "2px solid rgba(255,255,255,0.08)"
                : "2px solid #10b981",
              background: "transparent",
            ...
            <div style={{ display: "flex", alignItems: "start", gap: 16 }}>
              {state.business.photoUrl ? (
                ...
                <div style={{
                  ...
                  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgba(62,207,142,0.6)" strokeWidth="1.5">
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {state.business.detectedIndustry && (
                  <div style={{
                    ...
                    {state.business.detectedIndustry.replace('_', ' ').replace('-', ' ')} • {Math.round(state.business.confidence * 100)}% match
                  </div>
                )}
              </div>
            </div>
          ...
          <div style={{ padding: 24, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              {location && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {state.business.formattedAddress || state.business.address || `${location.city}, ${location.state} ${location.zip}`}
                  </div>
                </div>
              )}
            ...
            {state.business.detectedIndustry ? (
                ...
                <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{
                      ...
                      {state.business.detectedIndustry.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase())}
                    </div>
                  </div>
                </div>
                <div style={{
              ...
              <div style={{
                ...
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(251,191,36,0.9)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <circle cx="12" cy="12" r="10"/>
                  <line x1="12" y1="8" x2="12" y2="12"/>
                  <line x1="12" y1="16" x2="12.01" y2="16"/>
                </svg>
                <div style={{ flex: 1, fontSize: 12, color: "rgba(251,191,36,0.9)", lineHeight: 1.5 }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(59,130,246,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/>
                  <circle cx="12" cy="10" r="3"/>
                </svg>
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 10, color: T.textMuted, marginBottom: 2, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  Location
                </div>
                {state.business?.address && (
                  <div style={{ fontSize: 13, color: T.textPrimary, fontWeight: 500, marginBottom: 3 }}>
                    {state.business.address}
                  </div>
                )}
                <div style={{ fontSize: 14, color: T.textPrimary, fontWeight: 600 }}>
                  {location?.city && location?.state ? `${location.city}, ${location.state}` : location?.zip}
                </div>
              </div>
            </div>
