...
/** Derive wizard phase from step: Steps 1-3 = modeling, Step 4 = truequote */
function getPhase(step: WizardStep): "modeling" | "truequote" {
  return step === "results" ? "truequote" : "modeling";
}
...
import Step4MagicFitV7 from "@/components/wizard/v7/steps/Step4MagicFitV7";
 ...
 *
 * PROPS (Feb 8, 2026):
 * - initialIndustry: Pre-select industry when launched from vertical landing pages
 */

interface WizardV7PageProps {
  /** Pre-select industry slug when launched from a vertical landing page (e.g., 'car_wash', 'hotel', 'ev_charging') */
  initialIndustry?: string;
}

export default function WizardV7Page({ initialIndustry }: WizardV7PageProps = {}) {
  ...
  // ---------------------------------------------------------------------------
  // PRE-INIT from vertical landing page (Feb 8, 2026)
  // When launched from a vertical landing page, pre-select the industry so the
  // user sees it already highlighted when they reach Step 2.
  // ---------------------------------------------------------------------------
  const industryPreSelected = useRef(false);
  useEffect(() => {
    if (!initialIndustry || industryPreSelected.current) return;
    if (state.industry === "auto" && !state.isHydrating) {
      industryPreSelected.current = true;
      selectIndustry(initialIndustry as IndustrySlug);
      if (import.meta.env.DEV) {
        console.log("[V7] Pre-selected industry from landing page:", initialIndustry);
      }
    }
  }, [initialIndustry, state.industry, state.isHydrating, selectIndustry]);
  ...
  const _handleJump = useCallback(
    (n: number | string) => {
      ...
      // 1=location, 2=industry, 3=profile, 4=magicfit, 5=results
      ...
      if (idx === 4) return void goToStep("magicfit");
      if (idx === 5) return void goToStep("results");
    },
    [goToStep]
  );
  ...
  const _stepMeta = useMemo(() => {
    const labelByStep: Record<string, string> = {
      ...
      magicfit: "Choose your power level",
      results: "Your TrueQuote™ options",
    };

    // progress.stepIndex is 0..4 now (was 0..3)
