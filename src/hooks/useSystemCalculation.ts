...
export function useSystemCalculation(params: SystemCalculationParams) {
  ...
  // 300ms debounce so rapid slider drags don't fire a full QuoteEngine call
  // on every pixel — keeps sensitivity analysis snappy (C5.3, March 2026)
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      calculateFromSSoT();
    }, 300); // 300ms debounce — fast enough for sensitivity sliders
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
    };
  }, [
