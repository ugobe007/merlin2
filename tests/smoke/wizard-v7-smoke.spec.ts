...
// ============================================================================
// SMOKE 0: ESCAPE HATCH (Founders never trapped)
// ============================================================================
test.describe("Smoke 0 — Escape Hatch", () => {
  test("should allow partial submission with incomplete Step 3", async ({ page }) => {
    await page.goto(`${BASE_URL}/wizard?debug=1&mockPricing=ok`);
    await page.waitForTimeout(500);
    
    // Step 1: Complete location
    await page.locator('input[placeholder*="address"]').first().fill("123 Main St, San Francisco, CA");
    await page.locator('button:has-text("Continue")').first().click();
    await page.waitForTimeout(500);
    
    // Step 2: Select industry
    await page.locator(SEL.industryCarWash).click();
    await page.waitForTimeout(500);
    
    // Step 3: Fill SOME questions but leave it incomplete
    // (Intentionally don't fill all required fields)
    ...
    // Primary "See Results" button should be disabled (incomplete)
    const primaryBtn = page.locator('[data-testid="step3-continue"]').first();
    const isDisabled = await primaryBtn.isDisabled();
    expect(isDisabled).toBe(true);
    
    // Escape hatch button should be visible and enabled
    const escapeBtn = page.locator('[data-testid="step3-continue-partial"]').first();
    await expect(escapeBtn).toBeVisible();
    const escapeDisabled = await escapeBtn.isDisabled();
    expect(escapeDisabled).toBe(false);
    
    // Click escape hatch
    await escapeBtn.click();
    await page.waitForTimeout(1000);
    
    // Should navigate to Step 4 (Results)
    await expect(page.locator('text=/Step 4.*Results/i')).toBeVisible({ timeout: 5000 });
    
    // Provisional warning banner should be visible
    await expect(page.locator('text=/Provisional Results/i')).toBeVisible({ timeout: 3000 });
    await expect(page.locator('text=/inputs missing/i')).toBeVisible();
    await expect(page.locator('text=/Complete Step 3/i')).toBeVisible();
    
    // Pricing should NOT be running (escape hatch = Layer A only)
    await waitForDebugPanel(page);
    const status = await getPricingStatus(page);
    // Should be "idle" (not pending/ok) since we didn't run pricing
    expect(status).toBe("idle");
    
    console.log("✅ Escape hatch worked: navigated with incomplete, pricing disabled");
  });
});
