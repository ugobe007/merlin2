-- =============================================================================
-- STEP 2: DEPLOY FULL SCHEMA - Tables, Indexes, Triggers, RLS, Initial Data
-- =============================================================================
-- This is the complete MASTER_SCHEMA.sql - run this file as-is
-- No cutting/pasting needed
-- =============================================================================
...
-- =============================================================================
-- SECTION 3: PRICING & CALCULATION SYSTEM
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS calculation_formulas (
    ...
    formula_category VARCHAR(50) NOT NULL,
...
-- =============================================================================
-- SECTION 4: VENDOR & MARKETPLACE TABLES
-- =============================================================================
...
-- =============================================================================
-- SECTION 5: SYSTEM CONFIGURATION & LOGS
-- =============================================================================
...
-- =============================================================================
-- NOTE: RLS Policies will be added in a separate script (04_ADD_RLS.sql)
-- This prevents circular dependency issues during initial table creation
...
VALUES (
    'bess_pricing_2025',
    'bess',
    '{"smallSystemPerKWh": 580, "mediumSystemPerKWh": 450, "mediumLargeSystemPerKWh": 350, "largeSystemPerKWh": 280, "smallSystemSizeMWh": 1, "mediumSystemSizeMWh": 5, "largeSystemSizeMWh": 15, "degradationRate": 0.02, "warrantyYears": 10, "roundTripEfficiency": 0.85}',
...
(
    ...
    'power_electronics',
    '{"inverterPerKW": 120, "transformerPerKVA": 80, "switchgearPerKW": 50, "protectionRelaysPerUnit": 5000}',
...
(
    ...
    'balance_of_plant',
    '{"bopPercentage": 0.12, "epcPercentage": 0.08, "laborCostPerHour": 85, "shippingCostPercentage": 0.03, "contingencyPercentage": 0.05, "urbanLaborPremium": 0.15, "skillLaborPremiumPercentage": 0.10}',
    'Balance of plant and EPC costs',
    'Industry standard percentages',
    'high',
    NULL
)
ON CONFLICT (config_key) DO UPDATE SET config_data = EXCLUDED.config_data, updated_at = NOW();
...
VALUES (
    ...
    '{"totalInvestment": {"type": "number", "unit": "USD", "description": "Total capital investment"}, "annualSavings": {"type": "number", "unit": "USD/year", "description": "Annual cost savings"}}',
...
(
    ...
    '{"totalInvestment": {"type": "number", "unit": "USD", "description": "Total capital investment"}, "totalSavings": {"type": "number", "unit": "USD", "description": "Total lifetime savings"}}',
...
(
    ...
    '{"powerMW": {"type": "number", "unit": "MW", "description": "Required power output"}, "durationHours": {"type": "number", "unit": "hours", "description": "Discharge duration"}, "roundTripEfficiency": {"type": "number", "unit": "decimal", "description": "Round-trip efficiency (0.85 typical)"}}',
...
ON CONFLICT (formula_key) DO UPDATE SET formula_expression = EXCLUDED.formula_expression, updated_at = NOW();
...
SELECT 
    uc.id, uc.name, uc.slug, uc.category, uc.description, uc.icon, uc.usage_count, uc.average_roi,
...
GRANT SELECT ON user_profiles, saved_projects, use_cases, use_case_configurations, pricing_configurations, calculation_formulas, market_pricing_data, vendors, vendor_products, system_config TO anon;

-- =============================================================================
-- SUCCESS MESSAGE
-- =============================================================================

SELECT '✅ MASTER SCHEMA DEPLOYED SUCCESSFULLY!' AS status,
       'All 28 tables created' AS tables,
       '3 pricing configs inserted' AS pricing,
       '3 calculation formulas inserted' AS formulas,
